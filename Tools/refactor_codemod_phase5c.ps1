#!/usr/bin/env pwsh
# Phase 5c codemod: rewrite Zenith_SceneManager:: call sites to use the
# subsystem accessors on g_xEngine. Only handles methods that are already
# 1-line forwarders. Methods with complex bodies stay on SceneManager for now
# and are migrated in a later step.
...
# === Routing table =========================================================
# Each entry: method name on Zenith_SceneManager -> (target subsystem accessor, method name on subsystem)
# Methods with the same name on both: just specify the accessor.

$routing = @{
    # SceneRegistry (g_xEngine.SceneRegistry())
    ...
    # SceneOperations (g_xEngine.SceneOperations())
    ...
    # SceneLifecycle (g_xEngine.SceneLifecycle())
    ...
    # SceneCallbacks (g_xEngine.SceneCallbacks()) - note: callback names drop the "Callback" suffix on the subsystem
    ...
    # SceneEntityOwnership (Zenith_SceneEntityOwnership:: - still namespace-style, retain as such)
    # We keep these as static-class calls since SceneEntityOwnership is stateless and
    # remains class-static per Phase 5a design.
}

# Class-style targets that use Zenith_SceneEntityOwnership:: rather than g_xEngine.X()
$ownershipMethods = @(
...
# Note: Path also excludes any output/intermediate dirs by checking for "\build\output" or "\Build\output".
...
foreach ($file in $files) {
    ...
    # Apply substitutions LINE BY LINE so we can skip lines flagged with a
    # special marker comment. Delegator bodies in subsystem .cpp files use
    # the marker `// CODEMOD_SKIP` so they don't get rewritten into infinite
    # recursion (delegator calls Zenith_SceneManager::Foo which IS the body).
