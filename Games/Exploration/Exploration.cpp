...
static void GenerateProceduralSplatmap(const Zenith_Image& xHeightmap, const std::string& strOutputPath)
{
	...
	Zenith_DataParallelTask xSlopeTask(ZENITH_PROFILE_ZONE("Flux Terrain"), ComputeSlopesTask, &xSlopeData, uNumInvocations, true);
	...
	Zenith_DataParallelTask xWeightTask(ZENITH_PROFILE_ZONE("Flux Terrain"), GenerateWeightsTask, &xWeightData, uNumInvocations, true);
...
static Zenith_Image GenerateProceduralHeightmap(uint32_t uSize, float fTerrainWorldSize)
{
	...
	Zenith_DataParallelTask xTask(ZENITH_PROFILE_ZONE("Flux Terrain"), GenerateHeightmapRowsTask, &xData, uNumInvocations, true);
...
static void InitializeExplorationResources()
{
	...
	for (u_int u = 0; u < 4; u++)
	{
		Resources().m_axTerrainMaterials[u] = Zenith_AssetRegistry::Create<Zenith_MaterialAsset>();
...
void Project_LoadInitialScene()
{
	...
	g_xEngine.Scenes().RegisterSceneBuildIndex(1, GAME_ASSETS_DIR "Scenes/Exploration" ZENITH_SCENE_EXT);
