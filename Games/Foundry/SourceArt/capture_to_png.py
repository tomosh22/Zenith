...
# Optional paths also support the Logistics and MapDefense captures.
args=sys.argv[sys.argv.index('--')+1:] if '--' in sys.argv else (sys.argv[1:] if sys.argv[0].endswith('.py') else [])
src=root/(args[0] if args else 'Build/artifacts/foundry/workshop.tga')
dst=root/(args[1] if len(args)>1 else 'Games/Foundry/Mockups/InEngine/Workshop-1920x1080.png')
