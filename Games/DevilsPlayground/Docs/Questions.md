...
- `Zenith_Windows_FileAccess::WriteFile` creates parent dirs (CI checkout has no `Games/DevilsPlayground/Assets/Scenes/`)
- `SET_MODEL_MATERIAL` EditorAutomation step warns + skips when no model is loaded (instead of asserting)

9 GPU-mandatory tests + DoubleDoor + Forge (11 total) marked `m_bRequiresGraphics = true`. Harness emits `"skipped": true` JSON; skip counts as pass.
