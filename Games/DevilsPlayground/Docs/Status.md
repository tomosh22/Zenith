...
**MVP-0.0.5** — Verify `pwsh.exe` on PATH (HUMAN_GATE per roadmap, or rewrite scripts to `powershell.exe`). The MVP-0.0.4 rewrite already moved `run_dp_tests.ps1` to ASCII-only (PS5.1-compat), so the "rewrite scripts" path is largely already done; MVP-0.0.5 may simplify to a verification + the slang.dll post-build copy step fix.

**Phase 0.0 progress:** MVP-0.0.1 + MVP-0.0.2 + MVP-0.0.3-skeleton + MVP-0.0.4 shipped. **MVP-0.0.3 LANDED AS SKELETON ONLY (`workflow_dispatch`-trigger; not gating PRs).** Reactivating as a PR gate is blocked on Q-2026-05-12-007 (GPU on CI). **MVP-0.0.6** (branch-protection on `dp-tests`) cannot land until that's resolved.
