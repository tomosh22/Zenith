...
**MVP-0.0.4** — Extend `Tools/run_dp_tests.ps1` with `-Tier`, `-FailFast`, `-AssertionsLog` flags. Add `Test_T0Harness_RunnerFlagsExist`. Note: also fix the script's em-dashes (Q-2026-05-12-005) while editing — PS5.1 compat.

**Phase 0.0 progress:** MVP-0.0.1 + MVP-0.0.2 + MVP-0.0.3-skeleton shipped. **MVP-0.0.3 LANDED AS SKELETON ONLY (`workflow_dispatch`-trigger; not gating PRs).** The runtime engine is Vulkan-mandatory and free GitHub runners have no GPU; the smoke step hung indefinitely on `vkEnumeratePhysicalDevices`. Reactivating as a PR gate is blocked on (a) paid GPU runner (vetoed), (b) Mesa lavapipe + engine code changes, or (c) a `--no-graphics` engine boot mode. See Q-2026-05-12-007. **MVP-0.0.6** (branch-protection on `dp-tests`) cannot land until this is resolved.
