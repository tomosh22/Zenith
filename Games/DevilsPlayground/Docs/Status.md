...
Follow-ups queued from this pass: minimap rotated-room rects; third-person camera wall-clipping avoidance; Forge-track effects once crafting has a duration; economy tuning of the Knot cost curve after playtests; FrontEnd orbit-camera usage confirmation was folded into the never-possessed camera tests.

**Engine fix (2026-07-02):** `Zenith_EditorAction` (`Zenith/Editor/Zenith_EditorAutomation.h`) now owns copies of its string args (`std::string` members instead of aliasing `const char*`), closing the dangling-pointer hazard that previously required `AuthorLiminalScene`'s `static std::string` workaround — that workaround has been reverted to plain per-iteration stack buffers (see `AuthorLiminalScene` in `DevilsPlayground.cpp`).
