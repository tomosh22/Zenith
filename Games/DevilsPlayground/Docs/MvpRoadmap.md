...
- [~] **MVP-0.0.3** — Author `.github/workflows/dp-tests.yml` that runs `Tools/run_dp_tests.ps1 -Headless` on PR. Required check name: `dp-tests`. Uploads test result JSON as artifact. *Skeleton landed 2026-05-12; **engine `--headless` mode shipped 2026-05-13** (Q-2026-05-12-007 partial resolution — Option C). dp-tests.yml triggers active (pull_request/push) but the check is **NOT a required branch-protection gate** — fresh CI checkouts hit `LoadModel: File does not exist` because `Games/DevilsPlayground/Assets/Meshes/` is gitignored. Engine softens the downstream `SET_MODEL_MATERIAL` assertion so authoring continues, but model-less entities can't satisfy most gameplay-test setups. Asset provisioning (placeholder-bundle / CC0 archive / self-hosted runner) is the next gate. See Q-2026-05-12-007 for the three follow-up options.*
...
- [x] **MVP-0.0.6** — ~~🚧 HUMAN_GATE.~~ Repo-protect master via `gh api` (token's `repo` scope was sufficient -- no admin gate, no web-UI step needed). Required status checks: `dp-build` + `complexity-gate` (shipped 2026-05-12). `dp-tests` was added briefly on 2026-05-13 then **reverted** the same day after the asset-provisioning gap surfaced; it remains an active informational check. Linear history required (matches squash-merge convention). `enforce_admins=false` (Tomos can bypass for emergencies; autonomous agents cannot). `allow_force_pushes` / `allow_deletions` off.
