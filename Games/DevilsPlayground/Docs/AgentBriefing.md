...
- **Status (2026-05-27):** Phase 1 + Phase 2 substantively complete; Phase 4 acceptance gates landed (4.1.1 / 4.3.1 / 4.3.2 / 4.3.3) -- only 4.3.4 🚧 HUMAN_GATE remains. Procgen migration complete 2026-05-19 (PRs #96-#117): hand-authored GameLevel + 4 gym scenes + UE-bridge pipeline retired; `DPProcLevel::Generate(seed, cfg)` is the only gameplay surface, bit-deterministic across `/fp:fast` configs. Telemetry v3 + seed-matrix tooling shipped 2026-05-20 (#120/#121); the 2026-05-22 game-balance pass (PR #141) added 4 new personalities (Magpie / Relay / Heretic / Trickster, 8 total) and set `night.reagents_required_for_victory = 3`; the 2026-05-23 personality unification (`0813cff6`) removed all per-personality buffs/nerfs so the 8 personalities now run identical bot mechanics; the 2026-05-26 doors-at-DoorPoints PR (`00bb2382`) ships the current 80 % matrix wins (64 / 80 cells) state with both user-ratified balance criteria met. Engine-wide `*Impl` removal + `g_xEngine.X()` facade collapse landed 2026-05-27 (`9e4253ca`) — no DP gameplay impact. **133 `ZENITH_AUTOMATED_TEST_REGISTER` invocations** across 113 .cpp files (some `#ifdef ZENITH_INPUT_SIMULATOR`-gated); see [Status.md](Status.md) Tests line for the live pass-rate caveat. Live frontier: Phase 3 assets (Mixamo spike -- HUMAN_GATE).
