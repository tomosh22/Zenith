...
/**
 * DPDoor_Component - two-way openable door, optionally key-locked.
 *
...
public:
	...
	// Setters called by DPProcLevelBootstrap between AddComponent and the
...
private:
	void HandleInteractInternal(Zenith_EntityID xVillager)
	{
		...
		case DoorAnim::Open:
		{
			...
			// runs BEFORE the door's (execution order = entity creation
			// order; pent spawned first), consuming the objective before
			// the door's check runs. By the time the door checks the
			// held tag, it's already None.
			//
