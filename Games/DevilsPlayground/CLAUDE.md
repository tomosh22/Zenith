...
Components/
  DPVillager_Component.h             # Villager SYSTEMS SHIM (W3): body config, velocity apply, materials,
                                     #   held visual, footstep emission verb. Decisions (Idle/Possessed/
                                     #   Fainted/Dead transitions, sprint/walk-quiet, life drain, footstep
                                     #   cadence) live in DP_Villager.bgraph (self-attached in OnAwake);
                                     #   accessors read its blackboard
  DPPlayerController_Component.h     # Per-frame tick spine (perception/cooldown/scent/channel/fog/night)
                                     #   + per-run state block. Input dispatch (click->pick->possess, G->
                                     #   drop) lives in DP_PlayerControl.bgraph ("PlayerTick")
  DPOrbitCamera_Component.h          # Orbit-camera script (CameraComponent is FPS-only)
  DPItemBase_Component.h             # Item SYSTEMS SHIM (W3): side-table register/unregister, tint,
                                     #   reagent-config resolve+mirror. Pickup decision chain (evaporate/
                                     #   cooldown/gates/channel/commit/bell) lives in DP_Item.bgraph
                                     #   ("ItemTick"; mutable countdowns on its blackboard)
  ...
  DPInteractable_Base.h            # Distance-based proximity + F-press → DP_OnInteract (plain base class, NOT a registered component)
    ...
    DPForge_Component.h              # Forge SYSTEMS SHIM (W3): interact plumbing + prefab spawn body.
                                     #   Craft decisions live in DP_Forge.bgraph (recipe/craftCount on its
                                     #   blackboard; SetRecipe/GetCraftCount read it)
  DP_GraphNodes.h                    # DP's Behaviour Graph node library (DPNode_* — see Components/CLAUDE.md)
  Priest_Component.h                 # Priest SYSTEMS SHIM (W3): nav agent ownership/wiring, perception
                                     #   registration + the perception->blackboard BRIDGE (writes the
                                     #   DP_Priest.bgraph blackboard), door opening, alert telegraphs.
                                     #   Decision body (reactive Selector apprehend>pursue>investigate>
                                     #   patrol) lives in DP_Priest.bgraph ("PriestTick"). The old
                                     #   Zenith_BehaviorTree + DP_BT_Nodes.h are DELETED
  ...
  DPPauseMenuController_Component.h  # Pause SYSTEMS SHIM (W3): persistent-scene singleton machinery,
                                     #   Victory/RunLost subscriptions, SetScenePaused + reset-then-load
                                     #   verbs. Esc/R/Q decisions live in DP_PauseMenu.bgraph ("PauseKeys";
                                     #   shown/runOver on its blackboard, which rides DontDestroyOnLoad)
...
Assets/
  ...
  Graphs/                            # Boot-authored Behaviour Graphs (regenerated every tools boot
                                     #   by AuthorBehaviourGraphs() in DevilsPlayground.cpp, all via
                                     #   Zenith_GraphBuilder / AddStep_GraphBuild since W3):
                                     #   DP_MainMenu / DP_Pentagram / DP_Chest / DP_NoiseMachine /
                                     #   DP_DoubleDoor / DP_Door / DP_Villager / DP_PlayerControl /
                                     #   DP_Item / DP_Forge / DP_PauseMenu / DP_Priest .bgraph
Config/
...
ALL gameplay decisions live in boot-authored Behaviour Graphs since the W3
conversion (doctrine: systems = C++ components, logic = graphs; runtime docs
in `Zenith/Scripting/CLAUDE.md`). `AuthorBehaviourGraphs()` in
DevilsPlayground.cpp regenerates every `.bgraph` each tools boot through
`Zenith_GraphBuilder` (`AddStep_GraphBuild`), before the scenes are authored.

| Graph | Driven by | Logic |
|---|---|---|
| `DP_MainMenu.bgraph` | `DPMenuRelay_Component` fires "MenuPlay"/"MenuQuit"/"MenuLiminal" | Play → `LoadSceneByIndex(1)`; Quit → `DPRequestQuit`; Liminal → index 2 |
| `DP_Pentagram.bgraph` | `DPGraphInteractable_Component` fires "Interact" | OnStart → win-state reset; deposit chain DECOMPOSED: gates → notify-BEFORE-consume → consume → `DP_OnObjectivePlaced` |
| `DP_Chest.bgraph` | same | Branch(isOpen) guard + `DPDispatchChestOpened`; lid `openT` via engine math (dt/duration, min-clamp), duration staged live via `DPReadTuningFloat` |
| `DP_NoiseMachine.bgraph` | same | emit priest-bait noise stimulus |
| `DP_DoubleDoor.bgraph` | same | open-guard + payload gate + `DPConsumeKeyForUnlock` + `DPDispatchDoorOpened`; per-frame leaf swing |
| `DP_Door.bgraph` | `DPDoor_Component` (shim) fires "Interact" | `SwitchOnInt(anim)`-anchored state machine: `DPDoorCheckKey` (sticky unlock + priest-probe spam guard) / `DPDoorPentagramDeferral`, anim writes, `DPDoorStateChanged`→`DPDoorEmitNoise`→event; state on the blackboard (`anim`/`openT`/`requiredKey`) |
| `DP_Villager.bgraph` | villager shim fires "VillagerTick" (dt payload) per frame | 4 chains in node order: state transitions (`SwitchOnInt`), sprint/walk-quiet booleans (sprint wins ties), life drain → `DPVillagerKill`, footstep cadence → `DPVillagerEmitFootstep`. Self-attached ×17 (+ test spawns) |
| `DP_PlayerControl.bgraph` | controller fires "PlayerTick" after its tick spine | Gate(clickPressed) → `DPPickVillagerUnderCursor` → `DPTryPossess`; Gate(dropPressed) → `DPDropHeldItem` |
| `DP_Item.bgraph` | item shim fires "ItemTick" (dt payload) per frame | the pickup early-return chain via chain-reuse events (ItemGate2/3, ItemCommit): evaporate, post-drop cooldown, possessed/held/range/child gates, reagent channel SM (pause-not-reset), commit + bell |
| `DP_Forge.bgraph` | forge shim fires "Interact" (packed villager) | `DPForgeCraft`: recipe gate, consume→spawn→equip, craftCount (bumps even on spawn fail), hammer audio, `DP_OnForgeCrafted` |
...
shims in OnAwake (idempotent, on-demand from pre-awake seeders like
`SetRecipe`/`SetTag` too) — the door's attach-BEFORE-`SetRequiredKey` rule
generalises to every blackboard-seeding setter. Node library:
...
(`DP_FindFirstEntityWithGraph`, `DP_GetGraphBool/Float`, ...) and the priest's
via `Priest_Component::ReadBB*` shim accessors.
