...
static bool Step_DoubleDoor(int /*iFrame*/)
{
	...
	case kDD_Verify:
	{
		...
		Zenith_SceneData* pxScene =
			g_xEngine.Scenes().GetSceneDataForEntity(g_xLeftLeaf);
		if (pxScene != nullptr)
		{
...
static const Zenith_AutomatedTest g_xDoubleDoorTest = {
	...
	// a Null build the model's GPU upload is short-circuited (VMA leaf
	...
	// Verify fails. Tagging skips the test in Null builds; the windowed
...
static const Zenith_AutomatedTest g_xForgeTest = {
	...
	// a Null build. Tagging skips it there; windowed coverage
