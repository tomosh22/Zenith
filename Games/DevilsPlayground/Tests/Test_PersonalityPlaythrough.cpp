...
namespace
{
	...
	std::string HPTempPath(const char* sz)
	{
		...
		char acPrefix[512];
		const bool bHasPrefix = Zenith_PlatformEnvironment::GetVariable(
			"DP_TEST_TMP_PREFIX", acPrefix, sizeof(acPrefix));
		std::string strPrefix = (bHasPrefix && acPrefix[0] != '\0')
			? std::string(acPrefix) + "_" : std::string();
