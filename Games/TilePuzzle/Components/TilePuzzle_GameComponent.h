...
/**
 * TilePuzzle_GameComponent.h - Main game coordinator
 *
...
namespace TilePuzzle
{
	struct TilePuzzleResources
	{
		...
		// Pinball_GameComponent reads them.
...
class TilePuzzle_GameComponent;

// User data for level select buttons
struct TilePuzzleLevelButtonData
{
	TilePuzzle_GameComponent* pxBehaviour;
...
struct HintSolverData
{
	...
	// std::atomic is neither copyable nor movable, which would make the owning
	// component non-movable (component pools relocate by move-construction).
	// Hand-rolled move keeps the component movable; only safe while no solver
	// task is in flight (the game never moves the component mid-solve).
	HintSolverData() = default;
	HintSolverData(HintSolverData&& xOther) noexcept
		: xLevelState(std::move(xOther.xLevelState))
		, axSolution(std::move(xOther.axSolution))
		, iResult(xOther.iResult)
	{
		bComplete.store(xOther.bComplete.load(std::memory_order_acquire), std::memory_order_release);
	}
};
...
class TilePuzzle_GameComponent
{
	...
	friend class TilePuzzle_AutoTestComponent;
#endif
public:
	TilePuzzle_GameComponent() = delete;
	TilePuzzle_GameComponent(Zenith_Entity& xParentEntity)
	...
	~TilePuzzle_GameComponent()
	{
		ClearHint();
	}

	// Component pools relocate by move-construction. Member-wise move is
	// correct here: this component is a per-scene singleton that is never
	// moved at runtime (added once per scene, never DontDestroyOnLoad'd),
	// so the raw owning m_pxHintTask pointer is never duplicated in practice.
	TilePuzzle_GameComponent(TilePuzzle_GameComponent&&) = default;
	TilePuzzle_GameComponent(const TilePuzzle_GameComponent&) = delete;
	TilePuzzle_GameComponent& operator=(const TilePuzzle_GameComponent&) = delete;
...
private:
	...
	// Parent entity (was on the old scripting base class before the
	// component-promotion migration)
	...
	void ActivateTargetCatHighlight(int32_t iShapeIndex)
	{
		...
		Zenith_SceneData* pxScene = m_xPuzzleScene.IsValid() ? g_xEngine.Scenes().GetSceneData(m_xPuzzleScene) : nullptr;
		if (!pxScene) return;
...
#include "TilePuzzle/Components/TilePuzzle_MetaGame.h"
	...
	void RemoveShape(size_t uLevelShapeIndex)
	{
		...
		if (m_xPuzzleScene.IsValid())
		{
			Zenith_SceneData* pxSceneData = g_xEngine.Scenes().GetSceneData(m_xPuzzleScene);
		...
		if (uNewlyEliminated != 0)
		{
			...
			for (size_t i = 0; i < m_xCurrentLevel.axShapes.size(); ++i)
			{
				...
				if (uOldEliminatedCount < xShape.uUnlockThreshold && uNewEliminatedCount >= xShape.uUnlockThreshold)
				{
					...
					if (m_xPuzzleScene.IsValid())
					{
						Zenith_SceneData* pxSD = g_xEngine.Scenes().GetSceneData(m_xPuzzleScene);
