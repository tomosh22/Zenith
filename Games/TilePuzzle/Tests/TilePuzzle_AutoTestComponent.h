...
/**
 * TilePuzzle_AutoTestComponent.h - Automated test suite for the Tile Puzzle game
 *
 * Implemented as a game ECS component with a per-frame state machine.
 * When --autotest is passed, this component is added on a persistent entity.
 *
...
class TilePuzzle_AutoTestComponent
{
public:
	TilePuzzle_AutoTestComponent() = delete;
	TilePuzzle_AutoTestComponent(Zenith_Entity& xParentEntity)
		: m_xParentEntity(xParentEntity)
	{
	}

	// Component contract - the test fixture has no persistent state
	...
	{
		ImGui::Text("TilePuzzle AutoTest runner");
	}
#endif
	...
	void OnUpdate(const float /*fDeltaTime*/)
	{
		...
		case PHASE_TEST_UI_ELEMENTS_EXIST:
			RunSingleTestLive("Test_UIElementsExist", &TilePuzzle_AutoTestComponent::Test_UIElementsExist);
			m_ePhase = PHASE_TEST_TRANSITION_SWITCH;
			m_uFrameDelay = 5;
			break;

		case PHASE_TEST_TRANSITION_SWITCH:
			RunSingleTestLive("Test_TransitionSwitch", &TilePuzzle_AutoTestComponent::Test_TransitionSwitch);
			m_ePhase = PHASE_TEST_VICTORY_INIT_HIDDEN;
			m_uFrameDelay = 5;
			break;

		case PHASE_TEST_VICTORY_INIT_HIDDEN:
			RunSingleTestLive("Test_VictoryInitHidden", &TilePuzzle_AutoTestComponent::Test_VictoryInitHidden);
		...
		case PHASE_TEST_UI_CACHED_POINTERS:
			RunSingleTestLive("Test_UICachedPointers", &TilePuzzle_AutoTestComponent::Test_UICachedPointers);
		...
		case PHASE_TEST_UI_SETTINGS_TOGGLES:
			RunSingleTestLive("Test_UISettingsToggles", &TilePuzzle_AutoTestComponent::Test_UISettingsToggles);
			m_ePhase = PHASE_TEST_UI_SCREEN_MANAGEMENT;
			m_uFrameDelay = 5;
			break;

		case PHASE_TEST_UI_SCREEN_MANAGEMENT:
			RunSingleTestLive("Test_UIScreenManagement", &TilePuzzle_AutoTestComponent::Test_UIScreenManagement);
			m_ePhase = PHASE_TEST_UI_CONFIRM_OVERLAY;
			m_uFrameDelay = 5;
			break;

		case PHASE_TEST_UI_CONFIRM_OVERLAY:
			RunSingleTestLive("Test_UIConfirmOverlay", &TilePuzzle_AutoTestComponent::Test_UIConfirmOverlay);
			m_ePhase = PHASE_TEST_UI_MENU_FOCUS_NAV;
			m_uFrameDelay = 5;
			break;

		case PHASE_TEST_UI_MENU_FOCUS_NAV:
			RunSingleTestLive("Test_UIMenuFocusNavigation", &TilePuzzle_AutoTestComponent::Test_UIMenuFocusNavigation);
			m_ePhase = PHASE_TEST_UI_STRETCH_ALL_BACKGROUNDS;
			m_uFrameDelay = 5;
			break;

		case PHASE_TEST_UI_STRETCH_ALL_BACKGROUNDS:
			RunSingleTestLive("Test_UIStretchAllBackgrounds", &TilePuzzle_AutoTestComponent::Test_UIStretchAllBackgrounds);
			m_ePhase = PHASE_TEST_UI_ALL_MENU_ELEMENTS;
			m_uFrameDelay = 5;
			break;

		case PHASE_TEST_UI_ALL_MENU_ELEMENTS:
			RunSingleTestLive("Test_UIAllMenuElements", &TilePuzzle_AutoTestComponent::Test_UIAllMenuElements);
			m_ePhase = PHASE_TEST_UI_ALL_SCREEN_ELEMENTS;
			m_uFrameDelay = 5;
			break;

		case PHASE_TEST_UI_ALL_SCREEN_ELEMENTS:
			RunSingleTestLive("Test_UIAllScreenElements", &TilePuzzle_AutoTestComponent::Test_UIAllScreenElements);
			m_ePhase = PHASE_TEST_UI_CONFIRM_DIALOG_FLOW;
			m_uFrameDelay = 5;
			break;

		case PHASE_TEST_UI_CONFIRM_DIALOG_FLOW:
			RunSingleTestLive("Test_UIConfirmDialogFlow", &TilePuzzle_AutoTestComponent::Test_UIConfirmDialogFlow);
			m_ePhase = PHASE_TEST_UI_PAGINATION;
			m_uFrameDelay = 5;
			break;

		case PHASE_TEST_UI_PAGINATION:
			RunSingleTestLive("Test_UILevelSelectPagination", &TilePuzzle_AutoTestComponent::Test_UILevelSelectPagination);
			m_ePhase = PHASE_TEST_UI_ECONOMY_DISPLAY;
			m_uFrameDelay = 5;
			break;

		case PHASE_TEST_UI_ECONOMY_DISPLAY:
			RunSingleTestLive("Test_UIEconomyDisplay", &TilePuzzle_AutoTestComponent::Test_UIEconomyDisplay);
...
private:
	...
	// ========================================================================
	TilePuzzle_GameComponent* m_pxPuzzleBehaviour = nullptr;
	...
	void RunSingleTestLive(const char* szName, bool (TilePuzzle_AutoTestComponent::*pfnTest)())
	{
		Zenith_Log(LOG_CATEGORY_UNITTEST, "[AutoTest] Running %s...", szName);
	...
	void UpdateFullGame_ResetSave()
	{
		...
		{
			Zenith_Log(LOG_CATEGORY_UNITTEST, "  FAIL: Could not find TilePuzzle_GameComponent on GameManager");
	...
	// Matches the inverse of TilePuzzle_GameComponent::ScreenToGrid.
	bool PuzzleGridToScreen(TilePuzzle_GameComponent* pxBehaviour, int32_t iGridX, int32_t iGridY, double& fScreenX, double& fScreenY)
	{
	...
	bool PinballWorldToScreen(Pinball_GameComponent* pxPinball, float fWorldX, float fWorldY, double& fScreenX, double& fScreenY)
	{
	...
	{
		Pinball_GameComponent* pxPinball = FindPinballBehaviour();
		if (!pxPinball)
		{
			m_uWaitFrames++;
			if (m_uWaitFrames > 120)
			{
				Zenith_Log(LOG_CATEGORY_UNITTEST, "  FAIL: Timed out waiting for Pinball_GameComponent");
				Zenith_Log(LOG_CATEGORY_UNITTEST, "[AutoTest] FAIL: Test_FullGame");
				m_uFailed++;
				Zenith_InputSimulator::Disable();
				m_ePhase = PHASE_COMPLETE;
	...
	void UpdateFullGame_PinballLaunch()
	{
		...
		if (!pxPinball)
		{
			Zenith_Log(LOG_CATEGORY_UNITTEST, "  FAIL: Lost Pinball_GameComponent during launch");
			Zenith_Log(LOG_CATEGORY_UNITTEST, "[AutoTest] FAIL: Test_FullGame");
			m_uFailed++;
			Zenith_InputSimulator::Disable();
			m_ePhase = PHASE_COMPLETE;
	...
	void UpdateFullGame_PinballPlaying()
	{
		...
		if (!pxPinball)
		{
			Zenith_Log(LOG_CATEGORY_UNITTEST, "  FAIL: Lost Pinball_GameComponent during play");
			Zenith_Log(LOG_CATEGORY_UNITTEST, "[AutoTest] FAIL: Test_FullGame");
			m_uFailed++;
			Zenith_InputSimulator::Disable();
			m_ePhase = PHASE_COMPLETE;
	...
	bool Test_UIElementsExist()
	{
		...
		TilePuzzle_GameComponent* pxBehaviour = FindPuzzleBehaviour();
		if (!pxBehaviour)
		{
			Zenith_Log(LOG_CATEGORY_UNITTEST, "  FAIL: Could not find TilePuzzle_GameComponent");
			return false;
		}
	...
	static bool Test_CheckAchievementsLogic()
	{
		...
		// Replicate CheckAchievements logic from TilePuzzle_GameComponent
	...
	bool Test_UICachedPointers()
	{
		...
		if (!pxBehaviour)
		{
			Zenith_Log(LOG_CATEGORY_UNITTEST, "  FAIL: No TilePuzzle_GameComponent found");
			return false;
		}
	...
	bool Test_UIScreenManagement()
	{
		...
		pxBehaviour->ShowScreen(TilePuzzle_GameComponent::SCREEN_MENU);
		...
		pxBehaviour->ShowScreen(TilePuzzle_GameComponent::SCREEN_LEVEL_SELECT);
		if (pxBehaviour->m_pxMenuBg && pxBehaviour->m_pxMenuBg->IsVisible())
		{
		...
		pxBehaviour->ShowScreen(TilePuzzle_GameComponent::SCREEN_HUD);
		if (pxBehaviour->m_pxHUDInfoGroup && !pxBehaviour->m_pxHUDInfoGroup->IsVisible())
		{
		...
		pxBehaviour->ShowScreenAdditive(TilePuzzle_GameComponent::SCREEN_VICTORY);
		if (pxBehaviour->m_pxHUDInfoGroup && !pxBehaviour->m_pxHUDInfoGroup->IsVisible())
		{
	...
	bool Test_UIAllScreenElements()
	{
		...
		pxBehaviour->ShowScreen(TilePuzzle_GameComponent::SCREEN_SETTINGS);
		{
			Zenith_UIComponent& xUI = pxBehaviour->m_xParentEntity.GetComponent<Zenith_UIComponent>();
			Zenith_UI::Zenith_UIElement* pxSettingsBg = xUI.FindElement("SettingsBg");
			if (pxSettingsBg && !pxSettingsBg->IsVisible())
		...
		pxBehaviour->ShowScreen(TilePuzzle_GameComponent::SCREEN_CAT_CAFE);
		{
			Zenith_UIComponent& xUI = pxBehaviour->m_xParentEntity.GetComponent<Zenith_UIComponent>();
	...
	void UpdateCatCafeSwipe_Init()
	{
		...
		if (!pxBehaviour)
		{
			Zenith_Log(LOG_CATEGORY_UNITTEST, "  FAIL: No TilePuzzle_GameComponent for cat cafe swipe test");
			m_uFailed++;
	...
	void ClickUIElementByName(const char* szName, TilePuzzle_GameComponent* pxBehaviour)
	{
		Zenith_UIComponent& xUI = pxBehaviour->m_xParentEntity.GetComponent<Zenith_UIComponent>();
	...
	void UpdateUIInteract_Init()
	{
		...
		if (!pxBehaviour)
		{
			Zenith_Log(LOG_CATEGORY_UNITTEST, "  FAIL: No TilePuzzle_GameComponent for UI interaction test");
...
TilePuzzleShapeDefinition TilePuzzle_AutoTestComponent::s_xTestSingleShape;
bool TilePuzzle_AutoTestComponent::s_bTestShapeInitialized = false;
