...
Test_PlayerControllerComponent::Test_PlayerControllerComponent(Zenith_Entity& xParentEntity)
...
static void UpdateCameraRotation(Zenith_CameraComponent& xCamera)
{
	...
	g_xEngine.Input().GetMousePosition(xCurrentMousePos);
...
void Test_PlayerControllerComponent::Shoot()
{
	...
	g_xEngine.Physics().SetLinearVelocity(xCollider.GetBodyID(), xFacingDir * 50.0f);
}

void Test_PlayerControllerComponent::OnUpdate(const float)
{
	...
	if (m_bFlyCamEnabled)
	{
		...
		if (g_xEngine.Input().IsKeyDown(ZENITH_KEY_SPACE))
		{
			Zenith_Maths::Vector3_64 xUp = xCamera.GetUpDir();
	...
	Zenith_Maths::Vector3 xFinalVelocity(0, g_xEngine.Physics().GetLinearVelocity(xCollider.GetBodyID()).y, 0);
	...
	g_xEngine.Physics().SetLinearVelocity(xCollider.GetBodyID(), xFinalVelocity);
	xTrans.SetRotation(Zenith_Maths::EulerRotationToMatrix4(static_cast<float>(-xCamera.GetYaw() * Zenith_Maths::RadToDeg), { 0,1,0 }));

	Zenith_Maths::Vector3 xOffsetXZ = Zenith_Maths::Vector3(sinf(static_cast<float>(xCamera.GetYaw())), 0, -cosf(static_cast<float>(xCamera.GetYaw()))) * dbg_fCamDistance;
	Zenith_Maths::Vector3 xPos;
...
void Test_PlayerControllerComponent::OnAwake()
{
	FindHUDElements();
	return;
...
void Test_PlayerControllerComponent::SetBulletPrefabPath(const std::string& strPath)
{
...
void Test_PlayerControllerComponent::FindHUDElements()
{
	if (m_bUIInitialized)
		return;
...
void Test_PlayerControllerComponent::UpdateHealthUI()
{
	if (!m_bUIInitialized)
		FindHUDElements();
...
void Test_PlayerControllerComponent::UpdateCompassUI()
{
	if (!m_bUIInitialized)
		FindHUDElements();
...
void Test_PlayerControllerComponent::UpdateInventoryUI()
{
	if (!m_bUIInitialized)
		FindHUDElements();
...
void Test_PlayerControllerComponent::SetHealth(float fHealth)
{
	m_fHealth = std::max(0.0f, std::min(fHealth, s_fMaxHealth));
	UpdateHealthUI();
}

void Test_PlayerControllerComponent::TakeDamage(float fDamage)
{
	SetHealth(m_fHealth - fDamage);
}

void Test_PlayerControllerComponent::Heal(float fAmount)
{
	SetHealth(m_fHealth + fAmount);
}

void Test_PlayerControllerComponent::SetSelectedSlot(int iSlot)
{
