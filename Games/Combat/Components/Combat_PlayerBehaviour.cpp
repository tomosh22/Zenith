#include "Zenith.h"
#include "Combat_PlayerBehaviour.h"
...
void Combat_PlayerBehaviour::OnAwake()
{
	// Register so the GameManager and other behaviours (enemy AI) can find us via
	// Combat_Behaviour::GetPlayerEntityID() instead of name scans.
	Combat_Behaviour::RegisterPlayer(m_xParentEntity.GetEntityID());
	m_xHitDetection.SetOwner(m_xParentEntity.GetEntityID());
	Zenith_Log(LOG_CATEGORY_GAMEPLAY, "[Combat_PlayerBehaviour] OnAwake on entity %u (registered)",
		m_xParentEntity.GetEntityID().m_uIndex);
}

void Combat_PlayerBehaviour::OnStart()
{
...
void Combat_PlayerBehaviour::OnDestroy()
{
	Combat_Behaviour::UnregisterPlayer(m_xParentEntity.GetEntityID());
}

void Combat_PlayerBehaviour::OnUpdate(float fDt)
{
...
void Combat_PlayerBehaviour::TriggerHitStun(float fDuration)
{
	m_xController.TriggerHitStun(fDuration);
}

void Combat_PlayerBehaviour::UpdateAttack(Zenith_TransformComponent& xTransform)
{
	...
	if (m_xController.IsAttacking() && m_xAnimController.IsAttackHitFrame())
	{
		...
		if (uHits > 0)
		{
			...
			Combat_Behaviour::NotifyComboHit(m_xController.GetComboCount(), 2.0f);
		}
	}
