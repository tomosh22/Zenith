...
namespace
{
	...
	bool Step_ZMBattleDirectorRoundTrip(int)
	{
		...
		case BDPhase::AwaitReady:
		{
			...
			g_uBDEntryGrassBlades    = g_xEngine.Grass().GetScheduledInstanceCount();
		...
		case BDPhase::AwaitInBattle:
		{
				...
				&& pxTransition->GetTransitionState() == ZM_BATTLE_TRANSITION_IN_BATTLE)
			{
				...
				g_uBDGrassInBattle = g_xEngine.Grass().GetScheduledInstanceCount();
		...
		case BDPhase::AwaitResume:
		{
			...
			g_uBDGrassAfter = g_xEngine.Grass().GetScheduledInstanceCount();
	...
	bool Verify_ZMBattleDirectorRoundTrip()
	{
		...
		if (g_bBDActive)
		{
			...
			if (!ZM_GrassBaseline::Restored(g_uBDGrassAtPark, g_uBDGrassAfter))
			{
				Zenith_Error(LOG_CATEGORY_UNITTEST,
					"[ZM_BattleDirectorRoundTrip] resumed grass blade count was %u "
					"against a parked %u -- either the field is EMPTY or it converged "
					"by more than %u tiles (see ZM_GrassBaseline.h for why this is not "
					"an equality)",
					g_uBDGrassAfter, g_uBDGrassAtPark,
