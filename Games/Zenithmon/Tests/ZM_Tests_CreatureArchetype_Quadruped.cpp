...
// ============================================================================
// ZM_Tests_CreatureArchetype_Quadruped -- S4 SC1 QUADRUPED archetype gate
// (suite ZM_Gen). The generic contract harness lives in ZM_Tests_CreatureGen.cpp;
// this file is the per-archetype gate for the ONE builder SC1 wires
// (ZM_BuildArchetype_Quadruped). It:
//   * runs the universal creature contract over the QUADRUPED species set
//     (mesh structure + the ZM_ValidateCreature rollup + same-seed determinism), and
//   * adds the QUADRUPED-specific STRUCTURAL assert: the built skeleton carries the
//     expected quadruped bones -- a single spine ROOT, four LEG roots, a TAIL, and a
//     HEAD -- located via ZM_GenMeshFindBone on the shared ZM_CreatureArchetypeCommon
//     kit naming convention (limb uppers end "Up" per ZM_AppendLimb; the spine prefix
//     is "Spine", the tail prefix "Tail", the head bone "Head").
//
// PURE / HEADLESS: no disk, no GPU, no ZENITH_TOOLS. Authored against the frozen
// seam, NOT the builder .cpp. As the roster shifts, the QUADRUPED species set is
// discovered dynamically (ZM_GetSpeciesData(id).m_eArchetype == QUADRUPED with a
// non-null builder), so the gate tracks whatever quadrupeds ship.
//
...
namespace
{
	bool IsQuadruped(ZM_SPECIES_ID eId)
	{
		const ZM_SpeciesData& xRow = ZM_GetSpeciesData(eId);
		return xRow.m_eArchetype == ZM_ARCHETYPE_QUADRUPED
	...
	// Same curated subset as the generic harness (all QUADRUPED in SC1): a 3-stage
	// family, a 2-stage family, a mid-dex 3-stage family, and two single-stage species.
	const ZM_SPECIES_ID g_aeSubset[] =
	{
		ZM_SPECIES_FERNFAWN, ZM_SPECIES_SYLVASTAG, ZM_SPECIES_STRAYLING,
		ZM_SPECIES_NIBBIN,   ZM_SPECIES_AURICORN,  ZM_SPECIES_DUNELEON,
	};
	constexpr u_int uSUBSET_COUNT = (u_int)(sizeof(g_aeSubset) / sizeof(g_aeSubset[0]));
}

// ############################################################################
// QUADRUPED structural assert -- the expected bone set
// ############################################################################

// Every QUADRUPED build carries the canonical quadruped skeleton: exactly one root
// which is a SPINE bone, at least four LEG roots (limb "...Up" bones from
// ZM_AppendLimb), a TAIL, and a HEAD -- all resolvable via ZM_GenMeshFindBone on the
// kit naming convention. Runs mesh-only (ZM_BuildCreatureMesh) over every quadruped.
ZENITH_TEST(ZM_Gen, Quadruped_ExpectedBoneSet)
{
	u_int uTested = 0u;
	for (u_int id = 0; id < (u_int)ZM_SPECIES_COUNT; ++id)
	{
		const ZM_SPECIES_ID eId = (ZM_SPECIES_ID)id;
		if (!IsQuadruped(eId)) { continue; }
		++uTested;
		...
		// Four leg roots (limb uppers).
		ZENITH_ASSERT_GE(CountBonesEndingWith(xMesh, "Up"), 4u,
			"species %u has fewer than 4 leg-root ('...Up') bones", id);

		// Tail + head.
		ZENITH_ASSERT_GE(CountBonesStartingWith(xMesh, "Tail"), 1u,
			"species %u has no tail bone", id);
		ZENITH_ASSERT_GE(ZM_GenMeshFindBone(xMesh, "Head"), 0,
			"species %u has no 'Head' bone", id);
	}
	ZENITH_ASSERT_GT(uTested, 0u, "no QUADRUPED species found -- structural gate vacuous");
}

// ############################################################################
// Universal harness over the QUADRUPED species set -- mesh structure
// ############################################################################

// The universal mesh contract (winding / bounds / weights / <=2 influences / bone
// caps / single-rooted parent-before-child skeleton) holds for every quadruped.
ZENITH_TEST(ZM_Gen, Quadruped_UniversalMeshStructure)
{
	u_int uTested = 0u;
	for (u_int id = 0; id < (u_int)ZM_SPECIES_COUNT; ++id)
	{
	...
	ZENITH_ASSERT_GT(uTested, 0u, "no QUADRUPED species found");
}

// ############################################################################
// Universal harness over the QUADRUPED subset -- full bundle rollup + determinism
// ############################################################################

// Over the curated quadruped subset, the full bundle passes the ZM_ValidateCreature
...
// for the QUADRUPED archetype specifically.
ZENITH_TEST(ZM_Gen, Quadruped_UniversalBundleAndDeterminism)
{
	u_int uTested = 0u;
	for (u_int i = 0; i < uSUBSET_COUNT; ++i)
	{
		...
		ZENITH_ASSERT_TRUE(xVal.m_bAllValid, "quadruped species %u failed the ZM_ValidateCreature rollup", (u_int)eId);
		ZENITH_ASSERT_TRUE(xVal.m_bShinyDiffers, "quadruped species %u shiny == base", (u_int)eId);
		ZENITH_ASSERT_TRUE(xVal.m_bIconDistinctTexels, "quadruped species %u icon lacks distinct texels", (u_int)eId);

		ZENITH_ASSERT_TRUE(ZM_CreatureBuildEqual(xA, xB),
			"quadruped species %u re-build not byte-identical", (u_int)eId);
		ZENITH_ASSERT_EQ(ZM_CreatureContentHash(xA), ZM_CreatureContentHash(xB),
			"quadruped species %u content hash diverged on re-build", (u_int)eId);
	}
	ZENITH_ASSERT_GT(uTested, 0u, "no QUADRUPED subset species were buildable");
}
