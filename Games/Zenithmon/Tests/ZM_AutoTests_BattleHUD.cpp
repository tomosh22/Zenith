...
namespace
{
	...
	bool Verify_ZMBattleHUD()
	{
		...
		if (g_bBDActive)
		{
			...
			{
				Zenith_Error(LOG_CATEGORY_UNITTEST,
					"[ZM_BattleHUD] resumed grass blade count was %u against a parked "
					"%u -- either the field is EMPTY or it converged by more than %u "
					"tiles (see ZM_GrassBaseline.h for why this is not an equality)",
					g_uBDGrassAfter, g_uBDGrassAtPark,
