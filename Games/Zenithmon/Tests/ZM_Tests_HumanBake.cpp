...
// (suite ZM_Gen). It bakes ONE model's bundle and proves (a) every per-model file
// landed non-empty, (b) every SHARED file the bundle refers to already exists, and
// (c) the baked .zmodel binds the SHARED ENGINE rig and self-lists the shared clip
// refs in order.
//
// ★ IT NO LONGER BAKES ANYTHING SHARED, AND THAT IS THE POINT. v5 called
// ZM_BakeHumanShared first, which wrote a game-owned .zskel and nine .zanim files.
// v6 binds engine:Meshes/StickFigure/, so the shared half is not baked by this game
// at all -- clause (b) checks those files are PRESENT rather than that this game
// PRODUCED them. A shared file that has to be baked before the test can see it
// would be a shared file this game still owns.
//
// The whole file body is guarded by ZENITH_TOOLS: ZM_BakeHuman only exists in _True
// configs (the _False header no-op returns false), so a _False / Android build sees
// an EMPTY translation unit here and links clean.
//
// SCOPE: ONE model (PlayerM, the first roster id) -- fast. The byte-identical
// re-bake invariant is deferred to the ZM_BakeManifest box (Roadmap S4); this smoke
// only proves the write path + the model's shared-rig/shared-clip binding.
// ============================================================================

#ifdef ZENITH_TOOLS

#include "Core/Zenith_TestFramework.h"
#include "Zenithmon/Source/Gen/ZM_HumanGen.h"          // ZM_BakeHuman, asset-path schemes, clip count
...
// Bake PlayerM's per-model bundle and assert every per-model file (.zmesh /
// _albedo.ztxtr / .zmtrl / .zmodel) landed non-empty, and that every SHARED file it
// binds already exists. If the bake environment is unavailable the bake returns
// false and the test SKIPS rather than fails (matching the creature-bake idiom) --
...
{
	// (a) The shared rig + clips must be present non-empty. NOT baked here: they are
	// engine assets, and this game only refers to them. The shared kinds are
	// contiguous 0..ZM_HUMAN_SHARED_ASSET_KIND_COUNT.
...
// Bake PlayerM's bundle, then load the baked .zmodel HERMETICALLY
...
// self-lists exactly the SHARED clip refs in enum order.
ZENITH_TEST(ZM_Gen, HumanBake_ModelBindsSharedRigAndClips)
{
	...
	// The model must bind the SHARED ENGINE skeleton ref
	// (engine:Meshes/StickFigure/StickFigure.zskel), proving humans carry neither a
	// per-model rig nor a game-owned copy of the shared one.
	char acSkelRef[512];
	...
	// It must self-list exactly the SHARED clip refs, in enum order.
	ZENITH_ASSERT_EQ(xModel.GetNumAnimations(), static_cast<u_int>(ZM_HUMAN_CLIP_COUNT),
		"baked .zmodel must self-list every shared clip");
	...
	if (xModel.GetNumAnimations() == static_cast<u_int>(ZM_HUMAN_CLIP_COUNT))
	{
		for (u_int c = 0; c < static_cast<u_int>(ZM_HUMAN_CLIP_COUNT); ++c)
		{
			...
			ZENITH_ASSERT_STREQ(xModel.GetAnimationPath(c).c_str(), acExpectRef,
				"baked .zmodel animation %u ref mismatch (order must follow the clip enum)", c);
		}
	}
}
