...
**Last updated:** 2026-07-13 (S3 -- Dawnmere scene now includes the verified
scene-owned Player/follow-camera pair under ZM-D-055; all outputs remain
git-ignored).
...
- **Scene-owned traversal pair:** the ignored `Dawnmere.zscen` now authors a
  `Player` at centre **(512, 26.9, 480)** with transform scale
  **(0.8, 1.8, 0.8)**, a dynamic generic capsule, and order-102
  `ZM_PlayerController`; its main camera carries order-103 `ZM_FollowCamera`
  with authored yaw 0. The centre is based on the real baked surface sample
  **Y=25.98577** at `(512,480)`: a 0.9 m capsule half-extent begins above that
  surface and falls inside the controller's 1.05 m ground probe. A SINGLE
  reload replaces both scene-owned entities, after which the camera reacquires
  the new generation. This generator-authored centre is not a spawn tag and
  does not pre-empt the pending `ZM_SpawnPoint`/`ZM_WarpTrigger` system.
...
Dawnmere.zscen (build index 2) now exist locally; Dawnmere is a controller-
traversable terrain/grass preview, but not yet a connected gameplay town. The
measured Thornacre and Route1 terrain families do **not** imply that their
scenes, trees, dressing, traversal, or gameplay content exist.
