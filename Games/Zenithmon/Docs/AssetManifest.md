...
**Last updated:** 2026-07-13 (S3 -- FrontEnd/Dawnmere now carry the verified
manager/spawn traversal infrastructure under ZM-D-056; all outputs remain
git-ignored).
...
- **Scene-owned traversal pair + feet marker:** the ignored `Dawnmere.zscen`
  ...
  **(0.8, 1.8, 0.8)**, a dynamic generic capsule, and order-102
  ...
  generation at the marker. No live `ZM_WarpTrigger` is authored in Dawnmere
  yet; PlayerHome/build 40 and the real door route are the next Roadmap box.
...
never committed. `FrontEnd.zscen` (build index 0) authors the manager-only,
non-transient `ZM_GameStateRoot` with order-104 `ZM_GameStateManager`; runtime
`OnStart` makes that entity persistent and retires duplicate managers.
`Dawnmere.zscen` (build index 2) authors the exact `TownCenter` feet marker and
scene-owned Player/camera described above. These files now prove a direct
playerless FrontEnd -> Dawnmere infrastructure request, but Dawnmere is still
not a connected gameplay town: no fade, PlayerHome/build 40 scene, or live
warp-trigger edge is authored. The measured Thornacre and Route1 terrain
families do **not** imply that their scenes, trees, dressing, traversal, or
gameplay content exist.
