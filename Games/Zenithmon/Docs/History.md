# Zenithmon -- Status History

**Document purpose:** the historical narrative that used to live in the back
half of [Status.md](Status.md), moved here VERBATIM on 2026-08-18 so that file
could return to the ~25-line budget its own template specifies and
`AgentBriefing.md` section 2.3 asks for.

**Nothing here is live work.** It is the S0-S7 record, the ZM-D-168 follow-up,
the S7 closure detail and the standing lessons each of those left behind. The
live state is [Status.md](Status.md); the plan is [Roadmap.md](Roadmap.md);
the reasoning trail is [DecisionLog.md](DecisionLog.md); and the work items are
on the `ZM` board (see [Board.md](Board.md)).

**Superseded prose is annotated, never deleted** -- the `[SUPERSEDED]`,
`[CORRECTED]` and `[UPDATED]` markers below are the convention, and they are
why this file is a move rather than a delete.

---

## Pre-2026-08-18 pin-block narrative (moved out of Status.md)

The prose that accumulated around the LIVE PIN block and the committed-asset
hash table. The PINS and the HASHES themselves stayed in
[Status.md](Status.md) -- they are the authority a gate is reconciled against.
This is the reasoning trail behind them.
...
`ReadPointer`) and the standard-control pointer-capture suite. See
