...
- **RULING B -- navmesh is NOT in S6. Authored waypoints. (Best-guess; the loop PROCEEDS.)** `ZM_NpcWalker` ships as a deterministic 2-waypoint patrol with a fixed dwell, driven by `SetLinearVelocity` -- no navmesh, no A*, no RNG. **Evidence:** `Zenith_AINavGeometry::GenerateFromScene` collects **only** `RIGIDBODY_TYPE_STATIC` colliders and models every one as a box OBB, so Dawnmere's untransformed `COLLISION_VOLUME_TYPE_TERRAIN` collider degenerates to a 1 m cube at the origin -- the generated mesh has roofs and pillar caps but **no ground**, and the player spawns off-mesh entirely. That is a missing engine feature, not a tuning problem, and fixing it would be an ENGINE change -> 4-game cross-regression + a 1097-baseline bump, grossly disproportionate for a town pedestrian. The generator also hard-clamps to a 1024x1024 voxel grid against Dawnmere's 1024 m domain, forcing cells coarser than the 0.8 m player capsule; and `.znavmesh` `LoadFromFile`/`SaveToFile` have **zero in-repo callers**. **The S6 gate contains no clause motion can help pass.** **Cost if wrong:** `ZM_StepWalker` (~120 lines + ~15 units) is superseded and deleted at S7; the `ZM_Interactable` interface, the NPC table, all four walk-up tests and the consolidated gate are unaffected, because the walker sits behind `SetWanderEnabled(bool)` and a per-frame velocity write.
