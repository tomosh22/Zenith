...
- **★ THE LIVE BASELINE IS IN `Status.md`'s TOP BLOCK. READ IT, NOT THIS PARAGRAPH.** As of
  ...
  **3127 ran / 3125 passed / 0 failed / 2 skipped**, engine boot **1502**. (+4 engine units
  ...
  deleted, 14 added — +3 for Phase-5 shadow casting, +4 for Phase-6
  displacement re-anchor pins, and +6 for Phase-7 types authoring.) (The pre-grass 2026-08-06 figures were
...
**Board:** the items here are issues on the `ZM` board, under three epics —
`ZM-15` (tech debt & shortfalls, sections 1 and 3), `ZM-14` (engine gaps, section 2)
and `ZM-18` (visual polish). This document stays the NARRATIVE; the board is what can
be scheduled, blocked and reported on. See [Board.md](Board.md).
...
| # | Board | Gap | Why it blocks Zenithmon | Lands at |
|---|-------|-----|------------------------|----------|
...
| E7 | `ZM-48` | **DEFERRED future work -- navmesh OPTION B (ZM-D-145).** A RUNTIME-GENERATED overworld nav path: (a) build the navmesh on scene load from the terrain **COLLISION** mesh -- physics is live headless (ZM-D-127), so this sidesteps the render-side Q-2026-07-21-001 GPU-culling assert that blocks harvesting the live *render* terrain; (b) **TILING** so `Zenith_NavMeshGenerator`'s single-grid `iMaxDim=1024` clamp stops bounding resolution over a large domain (finer than ~1 m over 1024 m) -- this is exactly what UE (Recast `NavMeshBoundsVolume`) and Unity (`NavMeshSurface`, Terrain as a native source) ship for terrain worlds; plus (c) actual agent routing/local avoidance. Closely related to E6 (the fixed-4096 m grid). **Zenithmon does NOT need this now:** S7 trainers use straight-line approach (genre-authentic), and persistence **SHIPPED** via option C (committed disk-baked `.znavmesh` + the reusable engine bake/load feature, ZM-D-147/SC1b), which needs none of B. Lands as its own explicitly-gated ENGINE sub-commit (ZenithAI/terrain, owing the full engine-unit + RenderTest-regression + cross-game sweep) when populated-world NPC pathing/obstacle-avoidance actually needs it. | Roaming/obstacle-avoiding overworld NPC navigation in populated towns (beyond scripted straight-line trainers) | S9/S10 (or post-ship) |
| E6 | `ZM-47` | **DEFERRED, post-Zenithmon TODO.** Terrain world-space extent is a global compile-time constant (`Flux_TerrainConfig::CHUNK_GRID_SIZE`/`CHUNK_SIZE_WORLD`/`TERRAIN_SIZE`, `Flux_TerrainConfig.h:27-36`) -- every terrain is a fixed 4096x4096 m grid; density is likewise a fixed constant (`fLowLODDensity`, `Zenith_TerrainComponent.cpp:493`), not a per-instance field. E2's rect export only crops that same fixed grid, it does not resize it, so a tiny route and a large city are forced to the same world-space size today. Fix requires the grid constants to become per-instance serialized fields + dynamic GPU/CPU buffers in the streaming manager + a decoupled density field -- explicitly out of scope for Zenithmon per the E2 rationale ("compile-time constants pervade streaming/grass"). **Keep in mind on every terrain-touching task through the rest of development -- do not build content-side workarounds assuming this changes mid-project.** Revisit as a dedicated engine initiative after Zenithmon ships. | Post-S12 |
| E8 | `ZM-49` | **TIER 1 LANDED 2026-08-21 (ZM-49). TIER 2 still DEFERRED** (booked 2026-08-01, ZM-D-173, task_0515a49e). `Zenith_TerrainComponent` now exposes `TryGetGroundHeightAt(worldX, worldZ, heightOut)`, plus the static cores `TryGetGroundHeightFromGeometry(const Flux_MeshGeometry*, ...)` and `TryGetGroundHeightFromTriangles(positions, vertexCount, indices, indexCount, ...)`: it finds the triangle in `GetPhysicsMeshGeometry()` whose XZ projection contains the point and interpolates **barycentrically** across it, so an interior point gets the exact plane height rather than a nearest-vertex approximation. No Jolt, no body filtering, no allocation. **★★ IT ANSWERS THE COLLISION SURFACE, NOT THE ONE THE PLAYER SEES, AND THAT IS A CONTRACT LIMIT RATHER THAN A FOOTNOTE.** `GetPhysicsMeshGeometry()` is baked at **4 m** quads; the HIGH render mesh is baked at **1 m** quads (`Zenith_TerrainChunkLayout::uPHYSICS_CHUNK_VERTEX_COUNT == CalculateChunkVertexCount(4u)` against `uHIGH_CHUNK_VERTEX_COUNT == CalculateChunkVertexCount(1u)`, and that header states the gap is deliberate: *"collision deliberately remains lower density than the nearby HIGH render mesh"*). The returned height is therefore the **4 m chord** across the visible surface -- **below** it over a convex ridge, **above** it in a concave dip, equal only where the ground is planar across the quad, with the error bounded by local relief over 4 m. Anything placed at it sinks into or floats over the drawn terrain by that much. **Failure is the `[[nodiscard]]` bool, never the value** -- and there are **THREE** false cases, not two: absent physics geometry, an XZ outside the mesh footprint, **and an XZ over a HOLE INSIDE it** (`CombineTerrainChunkGridCore` skips any chunk whose source mesh fails to load, so a sparse bake leaves gaps the API cannot distinguish from being off the edge of the world -- read `false` as "outside the world" and a sparse bake reads as a *smaller* world). All three leave the out-param UNTOUCHED, so a caller cannot read a hole as height 0.0 (and a genuine ground height of 0.0 still arrives with `true`). That removes the raycast occlusion problem outright; the raycast answered a materially different question -- *what is the first BODY below this point* -- and the difference was never filterable: **(a)** `Zenith_Physics::Raycast` / `Zenith_PhysicsQuery::RaycastIgnoring` take exactly ONE ignore entity, so two overlapping bodies over a column are unmeasurable; **(b)** restarting the ray below a hit does not rescue it, because anything STANDING on the ground has its underside AT the surface and anything deliberately embedded (the Home shell sinks 0.05 m so no visible gap opens) has it BELOW the surface. **What TIER 1 does NOT buy:** first, a RENDER-accurate height (above) -- so it is not a general placement query. Second, it reads `m_pxPhysicsGeometry`, which only `LoadCombinedPhysicsGeometry` fills. A scene LOAD fills it (`ReadFromDataStream` calls it), so a runtime probe on a loaded terrain is answered; the editor's Add-Component path constructs the component through the deserialization ctor WITHOUT deserializing, so a probe on a freshly added terrain still MISSES -- it is not yet an authoring-time query. Cost is a linear scan over the mesh triangles behind a per-triangle XZ bounding-box reject: fine for one-off probes and physics-space placement, NOT for a per-frame per-agent query over a full terrain. **TIER 2 remains open, and it now has TWO jobs rather than one:** keep or load the heightfield (**note the runtime component holds NO height data** -- it loads baked mesh chunks, and `Terrain/<Set>/Height.ztxtr` is read only by the TOOLS editor path), which is what would make the query work **(a)** at AUTHORING time and **(b)** against the **RENDERED** surface -- the heightfield is the source both densities are sampled from, so it carries no 4 m chord error. Job (b) was implicit before TIER 1 landed and is explicit now, because TIER 1 closed (a)'s runtime half without touching (b) at all. | **Bitten 2026-08-01:** ZM-D-173's Home door jambs sit on the shell's own face, so two solid bodies stand over each jamb's column; their authored heights are derived from ground sampled 0.5 m away and the residual is bounded by local relief but genuinely UNMEASURED, because the column that would measure it is the one that cannot be probed. **★★ TIER 1 DOES NOT ANSWER THAT QUESTION, AND THE JAMB RESIDUAL REMAINS UNMEASURED.** The residual is a **visual** quantity -- how far the jamb sits from the ground the PLAYER SEES -- and TIER 1 reads the 4 m collision proxy, so a probe there returns a number wrong by the very quantity being measured. (It does give the column *a* probe, wherever the terrain physics geometry is loaded, and whether Zenithmon's authoring path has it loaded where the jambs are placed is still UNVERIFIED -- but that is a question about the COLLISION height and settling it would not close this row.) **Closing it needs TIER 2's job (b)**, a render-accurate height; TIER 2 would additionally retire the measure-once-freeze-as-a-constant workflow that `Source/World/ZM_DawnmerePlacement.h` (W5 + ZM-D-173) is built around. | TIER 1 **DONE 2026-08-21**; TIER 2 post-S12 |
...
- **★ CAMERA-RELATIVE MOVEMENT vs WORLD-SPACE TEST DRIVES -- FIXED 2026-07-20 (ZM-D-130/131), keep in mind for every new walk test.** Player movement is camera-relative (`ZM_PlayerController::BuildCameraRelativeDirection` off the main camera's facing dir) and `ZM_FollowCamera` re-aims at the player every frame from a LAGGING camera-to-player vector, so the world-space meaning of W/A/S/D **rotates as the player turns**. Every test-side `DriveTowardXZ` originally chose keys by comparing raw world dx/dz, which is correct ONLY for a single leg walked from rest. Measured failure: on a multi-leg walk needing a ~120-degree heading change the player ran a stable 45-degree WRONG heading at full run speed while holding W and died on the stall watchdog. Every copy now projects the desired world direction onto the LIVE camera basis (degenerating to the old behaviour at yaw 0). **Any new traversal helper must do the same** -- copy the version in `Tests/ZM_AutoTests_NpcServices.cpp`. **RESOLVED 2026-08-30 (ZM-D-218): there is ONE driver now**, `Tests/ZM_TestWalkDrive.{h,cpp}`, with eight units of its own. There were EIGHT byte-identical copies when they were finally counted -- this line said "three" the whole time -- so the camera-basis correction below had to be applied three separate times and the pursuit-curve finding landed on seven suites nobody had looked at. Each suite keeps a thin wrapper holding its local name and its own `Clear*Input()`, because those clears release different key sets; moving them out reddened six automated tests in one batch. **★ AND THE FIX LEFT A SECOND-ORDER PROBLEM THAT ONLY SURFACED AT ZM-D-218.** Projecting onto the live camera basis makes the heading correct on average, but the driver is still QUANTISED TO EIGHT DIRECTIONS (it holds W/A/S/D, never a steering angle) while the basis itself rotates under it, so the player walks a **pursuit curve that cuts the corner**: measured 4.44 m of lateral offset from the straight line over a 48.8 m approach. That is invisible until something MEASURES A BEARING rather than a position -- an armed trainer's 8 m / 60-degree sight cone does, and rival Vesper was correctly placed, exactly facing, armed and WATCHING with the player driven to 0.077 m of him and permanently blind, reported as *"the walk-up STALLED"*. A target due N/S/E/W of the walk's origin, or on an exact 45-degree diagonal, is a FIXED POINT of the driver and has zero offset; ordinary bearings in between do not. See [MapLayoutPlaybook.md](MapLayoutPlaybook.md) section 3.3.
...
- **★ THE TREE BRUSH CANNOT SEE THE GAME'S KEEP-OUT, AND TREE TRUNKS HAVE COLLIDERS (2026-08-30, ZM-D-218).** Woodland is painted by a TERRAIN tool (`AddStep_TerrainSetTreeBrush` + a `TreePaint` stroke), which knows nothing about `ZM_DawnmereDressing` -- so every refusal `ZM_ScatterDawnmerePropsStep` honours is invisible to it, while the trunks it places are exactly the kind of collider those refusals exist to keep off drive legs and trainer sight lines. A clump reached to within 2.4 m of an armed trainer with every scatter clause green. The coupling is maintained BY HAND in the clump table and enforced only by `TreeClumps_AreEntirelyOutsideTheTownKeepOut`; nothing stops a new map forgetting it. See [MapLayoutPlaybook.md](MapLayoutPlaybook.md) section 3.6.
- **★ NOTHING MECHANICALLY DISTINGUISHES A PAD FROM A SHELF.** `Flatten` is a rate (35%/dab, and a pad issues two dabs) while `SetHeight` assigns; a recipe that uses a pad to level gameplay-critical ground compiles, bakes, and is wrong by metres. The rule is documented at the `Zenith_TerrainBrushTool` enum and in the playbook, but it is a READING rule -- no test scans recipes for it.
