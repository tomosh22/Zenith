...
**Verdict at a glance (updated 2026-07-11, S2 box 2 COMPLETE):**
- **What works today:** boot skeleton + the **complete S1 data core** (§1.1) and a deterministic headless battle engine through all of **box 2** (§1.2): full move-effect delivery/stat/major-status/volatile execution, guarded RNG, exact event streams, engine-owned forced + voluntary switching, Gen-III/IV catch math, wild flee, and the 2,000-battle fuzz soak. 102 `ZM_Data` + 228 `ZM_Battle` unit tests are green. There is still no integrated playable loop: the S3/S5 overworld-to-battle layers remain, plus battle boxes 3-6.
- **What's designed but unbuilt:** S2 boxes 3-6 (abilities+weather, exp/level/evolution, AI tiers, breeding+tower), plus every downstream world/presentation/content stage -- see [Roadmap.md](Roadmap.md) for the sequence and gates.
...
**Status: box 2 COMPLETE -- S2 box 1 + all six box-2 sub-commits landed and green (2026-07-11); S11 tower balance later.**
Shipped in `Source/Battle/` (headless, seeded, deterministic, append-only `ZM_BattleEvent` stream; ZM-D-032/033/034/035): the **box-1 keystone**; SC1 executor seam; SC2 stat stages + STATUS-category dispatch + accuracy/evasion; SC3 multi/double hit, recoil, drain, healing, fixed/half-HP/OHKO delivery, and field/screen/hazard setters; SC4 all six majors with action gates/chip/cures, burn damage reduction, and paralysis speed; SC5 all ten battle-local volatiles, separate Endure, Swagger, charge/semi-invulnerable/recharge/lock-in execution, Leech Seed/Trap end-of-turn behavior, forced switching, and the shared `DoSwitch` primitive; and SC6 `ZM_CatchCalc` (Gen-III/IV four-shake capture + wild flee odds) plus the engine pre-move SWITCH/ITEM(catch)/RUN actions. The suite is **228 `ZM_Battle` tests green**, including exact-stream/RNG-order goldens and the 2,000-battle fuzz soak (half wild, periodic catch/run). **Still unbuilt (S2 boxes 3-6):** abilities via per-hook fn-pointer structs + weather modifiers (box 3), exp/level/evolution (box 4), ZM_BattleAI tiers (box 5), breeding + Battle Tower (box 6).
