...
**Verdict at a glance (updated 2026-07-12, S2 box 6 SC1 COMPLETE):**
- **What works today:** boot skeleton + the complete S1 data core and a deterministic headless battle engine through **box 6 SC1**: full move/status/catch/switch execution; all 50 abilities + weather; modern party-share EXP, EV caps/yields, current EXP, mid-battle level/stat/move learning, terminal level-evolution queuing; a pure four-tier opponent AI (`ZM_BattleAI`: RANDOM/GREEDY/SMART/CHAMPION); and deterministic breeding + daycare (`ZM_Breeding` / `ZM_Daycare`). Current game-unit inventory is **186 `ZM_Data` + 382 `ZM_Battle` + 2 `ZM_Boot`**; full boot gate **1638 ran / 1637 passed / 0 failed / 1 skipped**. There is still no integrated playable loop: S3/S5 remain.
- **What's designed but unbuilt:** S2 box 6 SC2 (`ZM_BattleTower` logic), plus every downstream world/presentation/content stage -- see [Roadmap.md](Roadmap.md).
...
**Status: boxes 1-5 COMPLETE + box 6 SC1 (`ZM_Breeding`/`ZM_Daycare`) COMPLETE (2026-07-12); box 6 SC2 (`ZM_BattleTower`) remains.**
`Source/Battle/` now contains the deterministic append-only battle engine; the complete move/status/catch/switch executor; weather and all 50 ability realizations; `ZM_ExpAndLevel`; `ZM_BattleAI`; and `ZM_Breeding` + `ZM_Daycare`. Box 4 adds four integer curves, derived progression accessors, current cumulative EXP, modern per-opponent party share, capped EV accumulation, mid-battle level/stat/move learning, and terminal level-evolution queuing/pure mutation (67 tests; award-off event/state/RNG identity preserved). Box 5 adds `ZM_BattleAI` -- a pure, side-effect-free four-tier chooser (`ZM_ChooseAction`) that reads state `const`, draws only its own RNG (RANDOM tier only), and perturbs no battle RNG/state/event (box-1..4 goldens byte-identical); 28 `ZM_Battle` tests. Box 6 SC1 adds `ZM_Breeding` + `ZM_Daycare` (`Source/Battle/ZM_Breeding.{h,cpp}` + `ZM_Daycare.{h,cpp}`) -- pure deterministic egg-gen (offspring = base-evo of the mother; K-IV inheritance 3/5; Stasis-Stone nature lock; mother ability; base-evo L1 learnset) + daycare (deposit <=2, 1 exp/step, 256-step egg threshold); a reduced model on the shipped data (archetype = egg-group proxy; no gender/egg-groups/egg-moves, Q-2026-07-12-004); 33 `ZM_Data` tests. The battle suite is **382 `ZM_Battle` tests**; `ZM_Data` is now **186**; the full boot baseline is **1638**. **Still unbuilt:** `ZM_BattleTower` logic (box 6 SC2).
