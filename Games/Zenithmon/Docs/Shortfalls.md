...
**Verdict at a glance (updated 2026-07-11, S2 box-2 SC5 complete):**
- **What works today:** boot skeleton + the **complete S1 data core** (§1.1) and a deterministic headless battle engine through box-2 SC5 (§1.2): full move-effect delivery/stat/major-status/volatile execution, guarded RNG, exact event streams, and engine-owned forced switching. 102 `ZM_Data` + 207 `ZM_Battle` unit tests are green. There is still no integrated playable loop: SC6 catch/pre-move actions/soak and the S3/S5 overworld-to-battle layers remain.
- **What's designed but unbuilt:** S2 box-2 SC6 and boxes 3-6, plus every downstream world/presentation/content stage -- see [Roadmap.md](Roadmap.md) for the sequence and gates.
...
**Status: IN PROGRESS -- S2 box 1 + box-2 SC1-SC5 landed and green (2026-07-11); S11 tower balance later.**
Shipped in `Source/Battle/` (headless, seeded, deterministic, append-only `ZM_BattleEvent` stream; ZM-D-032/033/034): the **box-1 keystone**; SC1 executor seam; SC2 stat stages + STATUS-category dispatch + accuracy/evasion; SC3 multi/double hit, recoil, drain, healing, fixed/half-HP/OHKO delivery, and field/screen/hazard setters; SC4 all six majors with action gates/chip/cures, burn damage reduction, and paralysis speed; and SC5 all ten battle-local volatiles, separate Endure, Swagger, charge/semi-invulnerable/recharge/lock-in execution, Leech Seed/Trap end-of-turn behavior, forced switching, and the shared `DoSwitch` primitive. The suite is **207 `ZM_Battle` tests green**, including exact-stream/RNG-order goldens and the original 50-battle fuzz smoke. **Still unbuilt:** box-2 SC6 (`ZM_CatchCalc`, voluntary SWITCH/ITEM/RUN, Trap restrictions, deterministic 2,000-battle soak); then abilities + weather modifiers (box 3), exp/level/evolution (box 4), ZM_BattleAI tiers (box 5), breeding + Battle Tower (box 6).
