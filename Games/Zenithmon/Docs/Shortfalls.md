...
**Verdict at a glance (updated 2026-07-12, S2 box 4 COMPLETE):**
- **What works today:** boot skeleton + the complete S1 data core and a deterministic headless battle engine through **box 4**: full move/status/catch/switch execution; all 50 abilities + weather; modern party-share EXP, EV caps/yields, current EXP, mid-battle level/stat/move learning, and terminal level-evolution queuing. Current game-unit inventory is **153 `ZM_Data` + 354 `ZM_Battle` + 2 `ZM_Boot`**; full boot gate **1577 ran / 1576 passed / 0 failed / 1 skipped**. There is still no integrated playable loop: S3/S5 remain.
- **What's designed but unbuilt:** S2 box 5 (`ZM_BattleAI`) and box 6 (breeding/daycare/tower), plus every downstream world/presentation/content stage -- see [Roadmap.md](Roadmap.md).
...
**Status: boxes 1-4 COMPLETE through `ZM_ExpAndLevel` (2026-07-12); boxes 5-6 remain.**
`Source/Battle/` now contains the deterministic append-only battle engine; the complete move/status/catch/switch executor; weather and all 50 ability realizations; and `ZM_ExpAndLevel`. Box 4 adds four integer curves, derived progression accessors, current cumulative EXP, modern per-opponent party share, capped EV accumulation, mid-battle level/stat/move learning, and terminal level-evolution queuing/pure mutation. It adds 67 tests (45 `ZM_Data`, 22 `ZM_Battle`) while preserving award-off event/state/RNG identity. The battle suite is now **354 `ZM_Battle` tests**; the full boot baseline is **1577**. **Still unbuilt:** `ZM_BattleAI` tiers (box 5) and breeding/daycare/Battle Tower logic (box 6).
