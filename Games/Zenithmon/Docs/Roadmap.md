...
- [ ] `ZM_HumanGen` -- one shared skeleton + one shared 9-clip set; ~35 models. **SC1 DONE** (ZM-D-074): 34-row roster + shared fixed 16-bone skeleton + frozen public seam + minimal valid mesh; +6 `ZM_Gen` units (boot 1866->1872). **SC2 DONE** (ZM-D-075): real six-part per-model StickFigure-derived loft with modest height/BUILD variation and MESH-only fixed-order RNG; +4 units (boot 1872->1876); regeneration + Vulkan Debug True build + unit/headless gates green; reviewer clean. Bundle inverts creatures: ONE shared `.zskel` + 9 shared `.zanim` + ~35 per-model mesh/material/model that bind them. Remaining: SC3 texture + attachment silhouette, SC4 the shared 9-clip curves, SC5 tools disk bake (+2 `ZM_GenCommon` bridges) = ticks this box, SC6 (optional) windowed showcase.
