...
- [ ] `ZM_CreatureAnimGen` -- 6 clips per archetype template (Idle/Walk/Attack/Special/Hit/Faint), instantiated + exported per species (~912 .zanim). **SC1+SC2 DONE** (ZM-D-068/069): frozen pure ALL-CONFIG seam + shared curve kit + pure driver + QUADRUPED+BIPED+AVIAN builders + 12 `ZM_Gen` units (boot 1847->1859), generic harness generalized to auto-cover every wired archetype; clips are pure `f(archetype,clip-id)` rotation-only -> byte-identical across every species of an archetype; remaining 5 archetype builders declared-but-unwired; tools bake is a `return false` stub. Remaining: SC3 Serpent+Aquatic (limbless), SC4 Insectoid+Blob, SC5 FloaterPlantoid + all-8 totality gate, SC6 bundle-bake wiring (+6 anim asset-kinds, `uZM_CREATUREGEN_VERSION` 2->3, tools smoke, baseline ratchet) = ticks this box, SC7 (optional) windowed playability gate.
