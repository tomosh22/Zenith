...
- [~] `ZM_CreatureGen` -- all 8 archetypes (QUADRUPED/BIPED/AVIAN/SERPENT/AQUATIC/INSECTOID/BLOB/FLOATER-PLANTOID), mesh+skeleton via StickFigure loft, palettes/patterns, shiny variants, dex icons (the single biggest work item, ~7-8.5k lines). **SC1 DONE** (ZM-D-060): frozen `ZM_CreatureGen.h` seam + core driver + shared `ZM_CreatureArchetypeCommon` kit + QUADRUPED reference builder + 12-invariant `ZM_Gen` harness (boot gate 1804 -> 1824). Remaining: SC2 (BIPED+AVIAN), SC3 (SERPENT+AQUATIC), SC4 (INSECTOID+BLOB), SC5 (FLOATER-PLANTOID + all-152 gate + `.zmtrl`/`.zmodel` bundle bake + windowed gallery)
