...
- [ ] `ZM_CreatureAnimGen` -- 6 clips per archetype template (Idle/Walk/Attack/Special/Hit/Faint), instantiated + exported per species (~912 .zanim). **SC1 DONE** (ZM-D-068): frozen pure ALL-CONFIG seam + shared curve kit + pure driver + QUADRUPED builder + 10 `ZM_Gen` units (boot 1847->1857); clips are pure `f(archetype,clip-id)` rotation-only -> byte-identical across every species of an archetype; other 7 archetype builders declared-but-unwired; tools bake is a `return false` stub. Remaining: SC2 Biped+Avian, SC3 Serpent+Aquatic, SC4 Insectoid+Blob, SC5 FloaterPlantoid + all-8 totality gate, SC6 bundle-bake wiring (+6 anim asset-kinds, `uZM_CREATUREGEN_VERSION` 2->3, tools smoke, baseline ratchet) = ticks this box, SC7 (optional) windowed playability gate.
