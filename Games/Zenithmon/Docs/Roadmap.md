...
- [ ] **S3 visual sign-off** -- automated implementation and the definitive post-overlay-hitch serial local gate are green (ZM-D-057), but the human must review the ignored captures under `Build/artifacts/zenithmon/s3/visual/`. Do not mark S3 complete or begin S4/S5 until that verdict is appended to DecisionLog.md.
