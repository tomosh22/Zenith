...
- [ ] `ZM_CreatureAnimGen` -- 6 clips per archetype template (Idle/Walk/Attack/Special/Hit/Faint), instantiated + exported per species (~912 .zanim). **SC1-SC5 DONE** (ZM-D-068..072): frozen pure ALL-CONFIG seam + shared curve kit + pure driver + ALL 8 archetype builders (Quadruped/Biped/Avian/Serpent/Aquatic/Insectoid/Blob/FloaterPlantoid -- dispatch TOTAL, totality gate) + 18 `ZM_Gen` units (boot 1847->1865), generic harness auto-covers all 8; clips are pure `f(archetype,clip-id)` rotation-only -> byte-identical across every species of an archetype; every one of the 152 species yields 6 valid rotation-only clips headless. Tools bake is still a `return false` stub. Remaining: SC6 bundle-bake wiring (+6 anim asset-kinds + basename cases, implement `ZM_BakeCreatureClips`, call from `ZM_BakeCreature` + `AddAnimationPath` x6 per `.zmodel`, `uZM_CREATUREGEN_VERSION` 2->3, tools bake smoke, TestPlan/AssetManifest updates, full 4-config+D3D12 matrix, baseline ratchet) = TICKS THIS BOX, SC7 (optional) windowed playability gate.
