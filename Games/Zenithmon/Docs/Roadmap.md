...
- [ ] `ZM_Breeding` + `ZM_Daycare` logic; `ZM_BattleTower` logic (level-50 clamp, rentals, streak scaling, AI escalation) -- **SC1 (`ZM_Breeding` + `ZM_Daycare`) COMPLETE** (ZM-D-045): pure deterministic egg-gen (offspring = base-evo of the mother; K-IV inheritance [3, or 5 w/ Heirloom Knot]; Stasis-Stone nature lock; mother ability; base-evo L1 learnset) + daycare (deposit <=2, 1 exp/step, 256-step egg threshold for a compatible pair). Reduced model on the shipped data (archetype = egg-group proxy; no gender / egg-groups / egg-moves -- Q-2026-07-12-004). 33 `ZM_Data` tests; boot baseline 1638. **SC2 `ZM_BattleTower` (consumes `ZM_BattleAI`) PENDING.**
