...
*Post-S2 (user-directed 2026-07-12, ZM-D-048):* **feature-complete breeding + gender** completes the box-6 SC1 reduced model to the full mainline breeding scope (Scope.md Section 1 already locks "breeding/eggs/daycare" + "mainline mechanics"). SC-A gender foundation DONE (13 tests; boot baseline 1676). Pending: SC-B (real egg groups + GLOOPET Ditto-analog + gendered compatibility + offspring = female/non-Ditto parent), SC-C (egg moves + ability/hidden-ability inheritance + a derived hatch-cycle accessor). Confirmed: Ditto = GLOOPET universal breeder; hidden abilities IN; shiny/Masuda deferred to S5+; hatch-cycle data now, overworld step-driving at S9.
