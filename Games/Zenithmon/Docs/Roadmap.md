...
- [ ] `ZM_HumanGen` -- one shared skeleton + one shared 9-clip set; ~35 models. **SC1 DONE** (ZM-D-074): roster data (`ZM_HumanData`, 34 rows -- GDD-canonical named cast + original trainer/townsfolk types), the shared 16-bone humanoid skeleton (`ZM_AppendSharedHumanBones`, identity bind rotation), the frozen `ZM_HumanGen.h` seam (recipe/asset-paths/determinism/validation + the 9-clip enum + golden metadata), a MINIMAL valid mesh, + 6 `ZM_Gen` units (boot 1866->1872). Bundle inverts creatures: ONE shared `.zskel` + 9 shared `.zanim` + ~35 per-model mesh/material/model that bind them. Remaining: SC2 per-model humanoid mesh loft, SC3 texture + attachment silhouette, SC4 the shared 9-clip curves, SC5 tools disk bake (+2 `ZM_GenCommon` bridges) = ticks this box, SC6 (optional) windowed showcase.
