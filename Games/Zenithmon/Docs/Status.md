...
**Build:** GREEN on the S7 item 3 SC1 diff -- `Build\regen.ps1` GREEN (new `Source/Nav/` folder), then `zenith build Zenithmon` (`Vulkan_vs2022_Debug_Win64_True`, tools debug) GREEN. SC1 is Games-only, additive, touches NO engine or existing production file; no configuration matrix or cross-game sweep is owed. The local gate is the direct-`master` landing authority; CI is the post-push backstop.
**Tests:** boot unit gate **2525 ran / 2524 passed / 0 failed / 1 documented skip** (the pre-existing unrelated `GraphComponent::RegistryWideNodeRoundTrip` quarantine) -- **+4 `ZM_Nav`** boot units from SC1 (2521 -> 2525); `zm-tests.yml` bumped **2521 -> 2525** from the OBSERVED line; the engine reference remains **1103** and `Tools/run_unit_gate.ps1`'s default is untouched. The automated registry is UNCHANGED at **42** (SC1 adds NO windowed/automated test); headless **42/42, 0 failed**. **★ Teeth mutation-proven:** flipping the coverage-grid winding reded units 1/2-control/3-control together (3 red) while unit 4 stayed green; restored GREEN. The full windowed batch was NOT re-run (SC1 is boot-units-only, touches no production/existing code, no windowed test changed -- a windowed regression is not possible from this diff). `%APPDATA%/Zenith/Zenithmon` verified EMPTY; no stray `zenithmon.exe`. S7 has no visual or human gate.

## Current task

**S7 item 3 IN PROGRESS -- SC1 DONE (ZM-D-144); the concrete next action is SC1b -- navmesh PERSISTENCE (option C, ZM-D-145) -- to be done by a FRESH SESSION from a handoff prompt.** Per user directive (ZM-D-145) NO other Zenithmon feature work proceeds until SC1b lands: the trainer vertical (SC2+) resumes only afterward. SC1b bakes a Dawnmere navmesh at tools time and `Zenith_NavMesh::SaveToFile()`s it to a COMMITTED `Games/Zenithmon/Assets/Navmesh/Dawnmere.znavmesh` (`.znavmesh` is now un-gitignored -> CI-loadable headless with no terrain/GPU), loaded at runtime via an engine holder. **Per user directive (ZM-D-145), option C ships as a REUSABLE ZENITH ENGINE FEATURE (game-agnostic bake+persist+load orchestration in `Zenith/AI/Navigation/`, the Unity-NavMeshSurface / UE-RecastNavMesh analog), with Zenithmon as its FIRST consumer -- so it owes the FULL ENGINE gate (engine unit tests + the engine baseline 1103 moving + a RenderTest boot regression + a Combat/DP/CityBuilder cross-game sweep), not just Zenithmon's.** Option B (runtime-generated/tiled nav path from terrain collision) is deferred future ENGINE work (Shortfalls E7). The full plan is in **ZM-D-143** (amended by ZM-D-145): **SC1 DONE** navmesh eval spike (`Source/Nav/ZM_NavEval.{h,cpp}` + 4 `ZM_Nav` boot units; flat coverage grid over Dawnmere's 1024 m sub-rect -> `GenerateFromGeometry` -> walkable navmesh) -> **SC1b NEXT** navmesh persistence (option C) -> SC2 `ZM_TrainerData` + `ZM_STORY_FLAG_RIVAL1_DEFEATED` (index 6) -> SC3 pure sight cone + trainer `ZM_BattleConfig` helper -> SC4 HUD Run-gate on `bCanFlee` (**prereq -- the flee-assert breaks the process otherwise; golden-preserving**) -> SC5 forced-battle entry (`ZM_OnTrainerEncounter` + 2nd `ZM_BattleTransition` sub) + prize/defeat write-back -> SC6 sight FSM + occlusion GLUE as a by-value member of `ZM_Interactable` (order 113, no new order) -> SC7 first `ZM_GraphAuthoring` `.bgraph` (trainer-defeat beat) -> SC8 rival Vesper (item 4). Eight design defaults are logged in **Q-2026-07-24-002** (adopted; Q-A navmesh=spike-not-shipped-nav, Q-B graph=minimal `.bgraph`, Q-D rival authored in Dawnmere are the ones most worth a user steer). The two engine risks (navmesh `iMaxDim=1024` clamp; Q-2026-07-21-001 headless-terrain assert) are DESIGNED OUT via the procedural grid + `cellSize>=1.0`. With item 2 COMPLETE (SC1-SC6), the trainer chain: a forward sight-CONE -> freeze input -> approach -> dialogue -> forced battle -> defeat flags + prize money, with the occlusion RAY entering as a probe filter in the GLUE layer so the pure cone picker stays unit-testable (the fixed architecture rule); the FIRST useful `ZM_GraphAuthoring` trainer-glue integration (deferred here from S6 per ZM-D-134 -- `ZM_GraphAuthoring` is still not written); and the terrain-backed navigation evaluation -- feed terrain triangles or a coverage grid to `Zenith_NavMeshGenerator::GenerateFromGeometry` across Dawnmere's 1024 m domain and decide `.znavmesh` persistence. **Watch the OPEN engine gap Q-2026-07-21-001** (terrain sets up GPU culling resources with no headless guard), which may force an engine-side headless path for any terrain-triangle harvest -- an engine change then owes engine unit tests + a RenderTest boot regression + the cross-game sweep (rebuild each game before testing; `zenith test` runs the STALE exe otherwise). Story-flag PRODUCERS (`ZM_SetStoryFlag` at real beats) also first land here / with S8 -- SC1 shipped the registry + the gated CONSUMER only. ECS order 113 remains the last occupied game order; next free is **114**. Screens stay by-value non-ECS presenters on `ZM_UI_MenuStack` (order 112), one arm per dispatch switch. Continue autonomously; the next human stop is the **S8 vertical-slice go/no-go**.
