...
- **Unattended runs:** the checked-in `.claude/settings.json` allowlists the
  ...
  **Spelling matters:** allow rules prefix-match the WHOLE command string, and
  the boot unit gate is allowlisted only as
  `pwsh -NoProfile -ExecutionPolicy Bypass -File Tools\run_unit_gate.ps1*` --
  the shorter `pwsh -NoProfile -File Tools\run_unit_gate.ps1 ...` matches no
  rule and prompts.
