...
void ZM_GameStateManager::OnUpdate(float)
{
...
void ZM_GameStateManager::ReadFromDataStream(Zenith_DataStream& xStream)
{
	...
	// manager's active-scene Player before OnStart retires the duplicate.
	ResetTransitionState(false);
	m_uIssuedLoadRequestCount = 0u;
}
	...
	const char* szSpawnTag)
{
	...
	// Input freezes immediately on acceptance; the actual SINGLE load is issued
	// once, by the next manager update.
