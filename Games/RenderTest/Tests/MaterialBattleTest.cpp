...
namespace
{
	constexpr float fCZ    = 300.0f;	// platform centre Z (matches RenderTest_Showcase)
	constexpr float fTOP_Y = 120.0f;	// platform top (matches RenderTest_Showcase::fPLATFORM_TOP_Y)
...
static bool Step_MaterialBattleTest(int iFrame)
{
	...
	// spheres).
	case 40:
		AimAt(256.0f, 148.0f, 286.0f, 256.0f, fGridY, fCZ + 1.0f, 60.0f);
		Shot("00_overview_top"); break;
	case 70:
		AimAt(256.0f, 127.0f, 274.0f, 256.0f, fTOP_Y + 1.0f, fCZ, 62.0f);
		Shot("01_overview_raking"); break;

	// Four column-band close-ups (each ~2-3 columns x all 5 rows), above-front.
	case 100: AimAt(BandX(0), 134.0f, 290.0f, BandX(0), fGridY, fCZ + 1.0f, 44.0f); Shot("02_band_left");     break;
	case 130: AimAt(BandX(1), 134.0f, 290.0f, BandX(1), fGridY, fCZ + 1.0f, 44.0f); Shot("03_band_midleft");  break;
	case 160: AimAt(BandX(2), 134.0f, 290.0f, BandX(2), fGridY, fCZ + 1.0f, 44.0f); Shot("04_band_midright"); break;
	case 190: AimAt(BandX(3), 134.0f, 290.0f, BandX(3), fGridY, fCZ + 1.0f, 44.0f); Shot("05_band_right");    break;
	...
	case 220:
		AimAt(256.0f, 123.0f, 285.0f, 256.0f, fTOP_Y + 1.0f, fCZ, 60.0f);
		Shot("06_grazing_front"); break;
	case 250:
		AimAt(236.0f, 130.0f, 292.0f, 258.0f, fGridY, fCZ + 1.0f, 60.0f);
