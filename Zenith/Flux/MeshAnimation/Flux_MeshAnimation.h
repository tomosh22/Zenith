...
public:
	...
	private:
        ...
        uint32_t GetPositionIndex(float animationTime)
        {
            ...
            {
                if (animationTime < m_xPositions.Get(index + 1).second)
                    return index;
            }
            return 0;
        }
        ...
        uint32_t GetRotationIndex(float animationTime)
        {
            ...
            {
                if (animationTime < m_xRotations.Get(index + 1).second)
                    return index;
            }
            return 0;
        }
        ...
        uint32_t GetScaleIndex(float animationTime)
        {
            ...
            {
                if (animationTime < m_xScales.Get(index + 1).second)
                    return index;
            }
            return 0;
        }
        ...
        glm::mat4 InterpolatePosition(float animationTime)
        {
            if (m_uNumPositions == 1)
                return glm::translate(glm::mat4(1.0f), m_xPositions.Get(0).first);

            uint32_t p0Index = GetPositionIndex(animationTime);
            uint32_t p1Index = p0Index + 1;
            float scaleFactor = GetScaleFactor(m_xPositions.Get(p0Index).second,
                m_xPositions.Get(p1Index).second, animationTime);
            glm::vec3 finalPosition = glm::mix(m_xPositions.Get(p0Index).first,
                m_xPositions.Get(p1Index).first, scaleFactor);
        ...
        glm::mat4 InterpolateRotation(float animationTime)
        {
            ...
            {
                auto rotation = glm::normalize(m_xRotations.Get(0).first);
                return glm::toMat4(rotation);
            }

            uint32_t p0Index = GetRotationIndex(animationTime);
            uint32_t p1Index = p0Index + 1;
            float scaleFactor = GetScaleFactor(m_xRotations.Get(p0Index).second,
                m_xRotations.Get(p1Index).second, animationTime);
            glm::quat finalRotation = glm::slerp(m_xRotations.Get(p0Index).first,
                m_xRotations.Get(p1Index).first, scaleFactor);
        ...
        {
            if (m_uNumScales == 1)
                return glm::scale(glm::mat4(1.0f), m_xScales.Get(0).first);

            uint32_t p0Index = GetScaleIndex(animationTime);
            uint32_t p1Index = p0Index + 1;
            float scaleFactor = GetScaleFactor(m_xScales.Get(p0Index).second,
                m_xScales.Get(p1Index).second, animationTime);
            glm::vec3 finalScale = glm::mix(m_xScales.Get(p0Index).first, m_xScales.Get(p1Index).first
                , scaleFactor);
    ...
    void Update(float fDt, bool bDebug = false)
    {
        ...
        g_xEngine.VulkanMemory().UploadBufferData(m_xBoneBuffer.GetBuffer().m_xVRAMHandle, m_axAnimMatrices, sizeof(m_axAnimMatrices));
    }
    ...
    private:
        ...
        Zenith_HashMap<std::string, AnimBone> m_xBones;
