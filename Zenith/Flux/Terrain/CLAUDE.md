...
GPU-driven terrain rendering with LOD streaming and frustum culling. Supports 4,096 chunks (64×64 grid) with 4 LOD levels. Key features:
- GPU compute shader culling with indirect rendering
- Priority-based LOD streaming with 256MB vertex budget
- Always-resident LOD3 fallback for guaranteed visibility
...
### Grid Layout
Terrain divided into 64×64 = 4,096 chunks. Each chunk is 64 world units square, giving 4,096×4,096 unit total terrain. Chunks indexed as `(x, y)` where both range 0-63.

### LOD System
Four detail levels with distance-based selection:
- **LOD0:** Highest detail (0-632m from camera)
- **LOD1:** Medium detail (632-1000m)
- **LOD2:** Low detail (1000-1414m)
- **LOD3:** Always-resident fallback (1414m+, never streamed/evicted)
...
```
[LOD3 Always-Resident Region][Streaming Region for LOD0-2]
```

**LOD3 Region:** Pre-loaded at initialization, fixed position at buffer start. Contains all 4,096 chunks at lowest detail. Never evicted or moved.

**Streaming Region:** 256MB vertex + 64MB index budget. LOD0-2 chunks allocated/deallocated dynamically as camera moves. Uses best-fit allocator with priority-based eviction.

**Critical Design:** LOD3 at start means its absolute offsets never change. Streaming allocator returns relative offsets within streaming region, converted to absolute by adding LOD3 size.
...
- For each chunk in active set:
  ...
  - If LOD not resident: stream in from file (max 8 per frame)
- Scan all chunks for eviction candidates:
  - If LOD resident but distance > threshold × 1.5: evict (max 16 per frame)
...
### Eviction Strategy
Priority-based: chunks farthest from camera evicted first. Uses distance × 1.5 hysteresis to prevent thrashing. Eviction frees allocator space immediately for reuse.
...
**Grid:**
- `CHUNK_GRID_SIZE = 64` (64×64 chunks)
- `CHUNK_SIZE_WORLD = 64.0f` (units per chunk)
- `TERRAIN_SIZE = 4096.0f` (total world size)

**LOD Thresholds (squared):**
- `LOD0_MAX_DISTANCE_SQ = 400000.0` (~632m)
- `LOD1_MAX_DISTANCE_SQ = 1000000.0` (1000m)
- `LOD2_MAX_DISTANCE_SQ = 2000000.0` (~1414m)
- `LOD3_MAX_DISTANCE_SQ = FLT_MAX` (always fallback)
...
### Buffer Offsets
Allocator returns relative offsets within streaming region. Convert to absolute for GPU by adding LOD3 size. Mixing up relative/absolute was source of past bugs.

### CPU/GPU Threshold Sync
Distance thresholds for LOD selection MUST match between CPU (streaming) and GPU (culling). Mismatch causes CPU to stream LOD2 while GPU expects LOD1, falling back to LOD3.

### LOD3 Guarantee
LOD3 must always be available. If streaming fails or LODs 0-2 not loaded, GPU falls back to LOD3. This prevents holes in terrain but looks low-detail.

### Hysteresis
Eviction uses distance × 1.5 threshold. Prevents thrashing when camera oscillates near LOD boundary. Without hysteresis, same chunks stream in/out repeatedly.
...
**Why LOD3 First?** Guarantees fallback available even if streaming fails or budget exhausted. Prevents terrain holes.
