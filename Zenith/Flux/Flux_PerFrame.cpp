...
{
	m_uFrameCounter = 0;
	m_uNumBeginCallbacks = 0;
	m_uNumEndCallbacks = 0;
}
...
void Flux_RendererImpl::RegisterBeginFrameCallback(OnFrameBeginFunc pfn, void* pUserData)
{
	...
	Zenith_Assert(m_uNumBeginCallbacks < FLUX_MAX_PERFRAME_CALLBACKS,
	...
	m_apfnBeginCallbacks[m_uNumBeginCallbacks] = pfn;
	m_apBeginUserData   [m_uNumBeginCallbacks] = pUserData;
	m_uNumBeginCallbacks++;
}
...
void Flux_RendererImpl::RegisterEndFrameCallback(OnFrameEndFunc pfn, void* pUserData)
{
	...
	Zenith_Assert(m_uNumEndCallbacks < FLUX_MAX_PERFRAME_CALLBACKS,
		"Flux_PerFrame: end-frame callback array overflow (max %u). Increase FLUX_MAX_PERFRAME_CALLBACKS in ZenithConfig.h.",
		static_cast<u_int>(FLUX_MAX_PERFRAME_CALLBACKS));
	m_apfnEndCallbacks[m_uNumEndCallbacks] = pfn;
	m_apEndUserData   [m_uNumEndCallbacks] = pUserData;
	m_uNumEndCallbacks++;
}
...
{
#ifdef ZENITH_DEBUG
	AssertSubscriberTalliesMatch(m_uNumBeginCallbacks, m_uNumEndCallbacks);
#endif
	const u_int uRingIndex = GetRingIndex();
	for (u_int u = 0; u < m_uNumBeginCallbacks; u++)
	{
		m_apfnBeginCallbacks[u](uRingIndex, m_apBeginUserData[u]);
	}
}
...
#endif
	const u_int uRingIndex = GetRingIndex();
	for (u_int u = 0; u < m_uNumEndCallbacks; u++)
	{
		m_apfnEndCallbacks[u](uRingIndex, m_apEndUserData[u]);
	}
}
