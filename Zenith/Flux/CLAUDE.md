...
This works because init/shutdown are dependency-safe in ANY order beyond "FluxGraphics first" — every subsystem's `Initialise`/`Shutdown` touches only foundation (FluxGraphics/memory/backend) + its own state, never another feature. Only the **render-graph declaration order** is load-bearing (producer-before-consumer; see the ORDERING note in `Flux_FeatureRegistry.h`), and that's the order you write the calls in. The only non-feature exceptions are 4 `AddSetupStep(...)` raw steps interleaved in the list (FluxGraphics/HDR transient creation, Skybox aerial perspective, final-RT layout transition) — these aren't features. `RegisterFeature` is constrained on the `FluxRenderFeature` C++20 concept (a feature type exposes at least `Initialise()`), so passing a non-feature accessor — or one that lost `Initialise` to a typo — is a clear compile error at the call site instead of a silently-registered all-null feature. **If a render system can't fit the one-liner, fix the render system** rather than adding bespoke registration.
