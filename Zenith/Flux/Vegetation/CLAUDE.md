...
4. **Type parameters** come from `game:Vegetation/GrassTypes.zdata`
   (`Zenith_GrassTypeTableAsset`), loaded once at `Initialise` before the first
   gather. A present-but-unreadable file warns and keeps the built-ins; a
   missing one is still a legal state, and the four seeded built-ins (Meadow /
   Tall / Dry / Flowers) stand in.

   **Every game with a tools boot now SHIPS one**, generated by
   `Tools/Zenith_Tools_GrassAssetExport.cpp` — see *Generated grass assets*
   below. It used to be true that no game shipped a table, which is why the
   three per-type texture slots had never been exercised end to end.
...
## Generated grass assets

`Tools/Zenith_Tools_GrassAssetExport.cpp` (`GenerateGrassAssets`, called from
`GenerateTestAssets` on every tools boot) produces the three per-type textures
and the table that binds them. **No engine change was needed**: the type table
has always carried the textures by PATH and `Flux_GrassImpl::ResolveTypeTexture`
has always resolved a path to a bindless slot at load — what was missing was
anything to point it at.

| Output | Where | Format | Sampler |
|---|---|---|---|
| `Grass_Blade_Vein.ztxtr` | `ENGINE_ASSETS_DIR/Vegetation/` | 256×64 RGBA8 **sRGB** + mips | REPEAT |
| `Grass_Blade_Gloss.ztxtr` | ″ | 256×64 RGBA8 **linear** + mips | REPEAT |
| `Grass_Clump_Ramp.ztxtr` | ″ | 256² RGBA8 **sRGB** + mips | CLAMP |
| `GrassTypes.zdata` | `GAME_ASSETS_DIR/Vegetation/` | `Zenith_GrassTypeTableAsset` | — |
| `GrassTypes.gen` | ″ | the generator's version marker | — |

The **textures are engine assets and the table is a game asset**, and that split
is forced: `Flux_GrassImpl` boot-loads exactly one path
(`szZENITH_GRASS_TYPE_TABLE_ASSET_PATH`, i.e. `game:`), while the pixels are
shared by every game that grows grass.

The four authored types are **Meadow / DryStraw / Trampled / Tussock**. Their
GEOMETRY ranges are the shipped built-ins' (Meadow ← 0, DryStraw ← Dry,
Tussock ← Tall), deliberately: every capture and tuning session so far is
against those numbers, and a table that re-skinned *and* re-shaped the grass
would make a regression impossible to bisect. What the table changes is
appearance — the three texture bindings, colour, roughness, translucency — plus
one silhouette the built-ins have no equivalent of (Trampled).

> **The table is AUTHORABLE, so the generator must not clobber an author.** The
> terrain editor's *Save grass types* writes the same path. The generator
> therefore writes only when there is no table at all, or when `GrassTypes.gen`
> says the table it wrote last is stale; **an existing table with no marker is
> treated as hand-authored and left alone**. Bumping
> `uGRASS_ASSET_EXPORT_VERSION` is what makes a tools boot re-bake.

Both blade maps are sampled through a REPEAT sampler and the gloss lookup
deliberately runs **past u = 1** (it is indexed by `height * GlossRepeat`,
default 4), so both are built on the wrapped integer lattice
(`Zenith_TerrainNoise::Tileable*`). The vein and ramp maps **multiply** the
blade's albedo, so both are authored close to white: a map centred on mid-grey
halves the brightness of every blade in the world, which reads as a lighting
change rather than as a texture bug. `GrassAssets.*` in
`Tools/Zenith_Tools_GrassAssetExport.Tests.inl` pins all of that, plus the
table's round trip through the same reader `LoadAuthoredTypeTable` uses.
...
- **The foliage MATERIALS now borrow it too.** The tree-leaf and bush-foliage
  materials are `MATERIAL_SHADING_SUBSURFACE` + `SetTwoSided(true)`
  (`Tools/Zenith_Tools_{Tree,Bush}AssetExport.cpp`), so the same skin-tuned
  wrap/back-scatter that lights a blade now lights a leaf card. Same
  approximation, same documented future step below.
- **A bush card's duplicated windings are now redundant.** The bush mesh emits
  each card twice with opposite windings to fake two-sidedness; the material's
  `TWO_SIDED_NORMAL_FLIP` is the real fix and also gets the back face's
  lighting normal right, which the duplication never did. The duplication is
  left in place (pinned by `BushAssets.EveryCardIsEmittedDoubleSided`) — halving
  that geometry is a follow-up, not a drive-by.
