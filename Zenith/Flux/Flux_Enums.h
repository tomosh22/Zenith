...
enum ResourceAccess
{
	...
	RESOURCE_ACCESS_READ_VERTEX_BUFFER, // Buffer-only: vertex-attribute fetch (VERTEX_INPUT stage). For a compute-written buffer bound as a vertex stream (Stage-5 skinned arena) — its barrier must reach VERTEX_INPUT, which READ_BUFFER_SRV's shader-stage barrier does NOT cover.
