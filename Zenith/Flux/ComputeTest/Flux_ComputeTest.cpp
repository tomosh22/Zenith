...
void Flux_ComputeTest::Shutdown()
{
	if (g_xComputeOutput.m_xVRAMHandle.IsValid())
	{
		...
		Flux_MemoryManager::QueueVRAMDeletion(pxVRAM, g_xComputeOutput.m_xVRAMHandle,
			g_xComputeOutput.m_pxRTV.m_xImageViewHandle, g_xComputeOutput.m_pxDSV.m_xImageViewHandle,
			g_xComputeOutput.m_pxSRV.m_xImageViewHandle, g_xComputeOutput.m_pxUAV.m_xImageViewHandle);
