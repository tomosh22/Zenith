...
{
	// Frame counter / ring index is owned by Flux_RendererImpl; this method
	// receives the current ring index directly (no longer pulled from the
	// swapchain). The swapchain itself reads GetCurrentFrameIndex() which is now
	// a thin wrapper over g_xEngine.FluxRenderer().GetRingIndex().
