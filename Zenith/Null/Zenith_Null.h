...
// no-op stub. No Vulkan / D3D12 / native graphics headers are referenced.
// ============================================================================
...
// ----------------------------------------------------------------------------
// Zenith_Null_Sampler — mirrors Zenith_Vulkan_Sampler (alias Flux_Sampler).
// Alias-only (no concept) — just the neutral public methods.
// ----------------------------------------------------------------------------
class Zenith_Null_Sampler
{
public:
	Zenith_Null_Sampler() = default;

	static void InitialiseRepeat(Zenith_Null_Sampler& /*xSampler*/) { }
	static void InitialiseClamp(Zenith_Null_Sampler& /*xSampler*/) { }
	static void InitialisePointClamp(Zenith_Null_Sampler& /*xSampler*/) { }
};

// ----------------------------------------------------------------------------
// Zenith_Null_VRAM — mirrors Zenith_Vulkan_VRAM (alias Flux_VRAM).
...
// ----------------------------------------------------------------------------
// Zenith_Null — mirrors Zenith_Vulkan (the device / Flux_PlatformAPI).
...
public:
	Zenith_Null() = default;
	~Zenith_Null() = default;
	Zenith_Null(const Zenith_Null&) = delete;
	Zenith_Null& operator=(const Zenith_Null&) = delete;
	...
	// (Zenith_Null.cpp): the body needs the full render-graph / pass / entry types
	...
	void WritePersistentViewImage(u_int /*uBinding*/, const Flux_ShaderResourceView& /*xSRV*/, const Zenith_Null_Sampler& /*xSampler*/) { }
	...
	// ===== VRAM Registry =====
	Flux_VRAMHandle RegisterVRAM(Zenith_Null_VRAM* /*pxVRAM*/) { Flux_VRAMHandle x; x.SetValue(ms_uDummyHandle++); return x; }
	Zenith_Null_VRAM* GetVRAM(const Flux_VRAMHandle /*xHandle*/) { return nullptr; }
	...
	// ImGui::Render()), and ImGui asserts hard without a live context. So the
	// null backend creates the context and initialises the RENDERER-AGNOSTIC
	// GLFW platform backend (ImGui_ImplGlfw_InitForOther); only the RENDERER
	// backend (ImGui_ImplVulkan_*) is absent, so the composed draw data is
	// simply never rasterised. This is what lets editor-driven automated tests
	// run on a headless Null build. Out-of-line (Zenith_Null.cpp): the bodies
	...
	uint64_t CreateImGuiTextureID(const Flux_ShaderResourceView& /*xView*/, const Zenith_Null_Sampler& /*xSampler*/) { return ms_uDummyHandle++; }
#endif
