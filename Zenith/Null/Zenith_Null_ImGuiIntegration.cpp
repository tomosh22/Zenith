...
//=============================================================================
// Null-backend implementation of the Flux ImGui integration.
//
// Editor code (viewport / content browser / material editor / terrain editor)
// registers textures with ImGui through this neutral seam. The Null backend
// renders nothing, so there is no descriptor set to allocate and nothing to
// defer-delete -- but the symbols MUST exist or every tools-enabled Null build
// fails to link, and the handles MUST look valid or the editor's IsValid()
// guards silently drop the panels they gate.
//
// Same dummy-handle philosophy as the rest of the null backend: hand back a
