...
// The null memory manager's buffer initialisers touch the Flux_*Buffer
...
// from Zenith_Null_MemoryManager.h (which is itself reached via Flux.h's backend
...
// compiled only in Null configs (the Vulkan source partition excludes Null/).
#include "Flux/Flux_Buffers.h"
#include "Null/Zenith_Null_MemoryManager.h"
...
void Zenith_Null_MemoryManager::InitialiseVertexBuffer(const void* pData, size_t uSize, Flux_VertexBuffer& xBufferOut, bool bDeviceLocal)
{
...
void Zenith_Null_MemoryManager::InitialiseIndexBuffer(const void* pData, size_t uSize, Flux_IndexBuffer& xBufferOut)
{
...
void Zenith_Null_MemoryManager::InitialiseConstantBuffer(const void* pData, size_t uSize, Flux_ConstantBuffer& xBufferOut)
{
	(void)pData; (void)uSize;
	xBufferOut.GetBuffer().m_xVRAMHandle.SetValue(ms_uDummyHandle++);
	xBufferOut.GetBuffer().m_ulSize = uSize;
...
void Zenith_Null_MemoryManager::InitialiseIndirectBuffer(size_t uSize, Flux_IndirectBuffer& xBufferOut)
{
	(void)uSize;
	xBufferOut.GetBuffer().m_xVRAMHandle.SetValue(ms_uDummyHandle++);
	xBufferOut.GetBuffer().m_ulSize = uSize;
...
void Zenith_Null_MemoryManager::InitialiseReadWriteBuffer(const void* pData, size_t uSize, Flux_ReadWriteBuffer& xBufferOut, bool bAlsoVertexBuffer /*= false*/)
{
...
void Zenith_Null_MemoryManager::InitialiseDynamicVertexBuffer(const void* pData, size_t uSize, Flux_DynamicVertexBuffer& xBufferOut, bool bDeviceLocal)
{
	(void)pData; (void)uSize; (void)bDeviceLocal;
...
void Zenith_Null_MemoryManager::InitialiseDynamicConstantBuffer(const void* pData, size_t uSize, Flux_DynamicConstantBuffer& xBufferOut)
{
	(void)pData; (void)uSize;
	for (u_int u = 0; u < MAX_FRAMES_IN_FLIGHT; u++)
	{
...
void Zenith_Null_MemoryManager::InitialiseDynamicReadWriteBuffer(const void* pData, size_t uSize, Flux_DynamicReadWriteBuffer& xBufferOut)
{
	(void)pData; (void)uSize;
	for (u_int u = 0; u < MAX_FRAMES_IN_FLIGHT; u++)
	{
