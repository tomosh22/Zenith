...
// ============================================================================
// NO-OP "null" memory-manager backend.
//
// Mirrors Zenith_Vulkan_MemoryManager (the Flux_MemoryManager alias under
// ZENITH_NULL_RENDERER). Its ONLY job is to COMPILE + LINK against the backend-neutral
...
public:
	...
	Zenith_Null_MemoryManager() {}
	~Zenith_Null_MemoryManager() {}
	Zenith_Null_MemoryManager(const Zenith_Null_MemoryManager&) = delete;
	Zenith_Null_MemoryManager& operator=(const Zenith_Null_MemoryManager&) = delete;
	...
	// Zenith_Null_MemoryManager.cpp: they touch the Flux_*Buffer wrapper types
	...
	// Nothing was ever written to a GPU, so a headless readback yields zeroes BY
	// CONSTRUCTION — the destination is filled rather than left untouched so a
	// caller reading uninitialised stack never mistakes garbage for GPU truth.
	// Any test asserting on real buffer contents is therefore windowed-only.
	...
	Flux_DepthStencilView CreateDepthStencilViewForLayer(Flux_VRAMHandle xVRAMHandle, const Flux_SurfaceInfo& xInfo, uint32_t uLayer, uint32_t uMipLevel) { (void)xInfo; (void)uLayer; (void)uMipLevel; Flux_DepthStencilView x; x.m_xImageViewHandle.SetValue(ms_uDummyHandle++); x.m_xVRAMHandle = xVRAMHandle; return x; }
