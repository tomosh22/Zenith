...
// Called by Zenith_Editor::EnterStopMode (a namespace, not a friend of this
// class), which is why the public forwarder exists at all -- the editor cannot
// reach the private MarkEntityStarted directly. Phase 10 (ECS leaf-extraction):
// this forwarder names no Editor symbol -- it only takes a Zenith_EntityID and
// calls the private member -- so the "#include Editor/Zenith_Editor.h" that
// used to sit at the top of this TU was dropped, severing the inbound Editor
// edge from the ECS core.
...
void Zenith_SceneData::InvalidateActiveInHierarchyCache(Zenith_EntityID xID)
{
	...
	while (axStack.GetSize() > 0)
	{
		...
		// Queue children for invalidation. Phase 5a: children come from the SLOT
		// (single source of truth), so this no longer needs any owner component.
...
void Zenith_SceneData::CollectResetHierarchy(Zenith_Vector<Zenith_EntityID>& axHierarchyOut)
{
	...
	// Sweep: pick up any active entity not reached from a root by the traversal above
	// (e.g. an entity detached mid-teardown, so it is no longer attached to a root).
	for (u_int u = 0; u < m_xActiveEntities.GetSize(); ++u)
	{
		Zenith_EntityID xID = m_xActiveEntities.Get(u);
		if (!EntityExists(xID)) continue;
...
Zenith_EntityID Zenith_SceneData::GetParentEntityIDUnchecked(Zenith_EntityID xID) const
{
...
void Zenith_SceneData::Reset()
{
	...
	// supported; use LoadScene(SINGLE) to transition between scenes and let the
	// scene system tear the old one down.
	Zenith_Assert(!m_bIsUpdating,
		"Reset() called during Update — unload your scene via LoadScene(SINGLE), "
		"not from within OnUpdate/OnFixedUpdate. Same-scene self-unload is not "
		"supported.");
...
void Zenith_SceneData::ScrubAndReset()
{
	...
	Zenith_Assert(m_iHandle != Zenith_SceneSystem::Get().GetPersistentSceneHandle(),
...
Zenith_EntityID Zenith_SceneData::CreateEntity()
{
	...
	while (Zenith_ECS_EntityStore().m_axFreeEntityIndices.GetSize() > 0)
	{
		uIndex = Zenith_ECS_EntityStore().m_axFreeEntityIndices.GetBack();
		Zenith_ECS_EntityStore().m_axFreeEntityIndices.PopBack();

		Zenith_EntitySlot& xSlot = Zenith_ECS_EntityStore().m_axEntitySlots.Get(uIndex);
	...
	if (!bFoundFreeSlot)
	{
		uIndex = Zenith_ECS_EntityStore().m_axEntitySlots.GetSize();
		uGeneration = 1;

		Zenith_EntitySlot xNewSlot;
		xNewSlot.m_uGeneration = uGeneration;
		xNewSlot.OccupySlot(m_iHandle);
		Zenith_ECS_EntityStore().m_axEntitySlots.PushBack(std::move(xNewSlot));
	}

	while (Zenith_ECS_EntityStore().m_axEntityComponents.GetSize() <= uIndex)
	{
		Zenith_ECS_EntityStore().m_axEntityComponents.PushBack({});
	}
...
{
	// Phase 5a: hierarchy is encoded on the SLOT (single source of truth). Children
	// live on the slot and survive their owner component being destroyed, but that
	// component's destructor-time slot-backed DetachAllChildren clears the slot child
	// list when the component goes away — so a component-destroyed entity has an
	// empty child list here (its children become roots, swept by CollectResetHierarchy).
	// Copy the list before recursing (recursion may free slots / mutate child vectors
	// via slot release).
	if (xID.m_uIndex < Zenith_ECS_EntityStore().m_axEntitySlots.GetSize())
	{
		Zenith_Vector<Zenith_EntityID> axChildIDs = Zenith_ECS_EntityStore().m_axEntitySlots.Get(xID.m_uIndex).m_xChildEntityIDs;
		for (u_int i = 0; i < axChildIDs.GetSize(); ++i)
		{
...
{
	Zenith_Assert(Zenith_ECS_IsMainThread(), "RemoveEntity must be called from main thread");
	// C9: see CreateEntity for rationale — render-task-ordering invariant.
	Zenith_Assert(!Zenith_AreRenderTasksActive(),
	...
	for (u_int i = 0; i < axHierarchy.GetSize(); ++i)
	{
		...
		xSlot.ReleaseSlot();
		Zenith_ECS_EntityStore().m_axFreeEntityIndices.PushBack(xEntityID.m_uIndex);
...
{
	Zenith_Assert(Zenith_ECS_IsMainThread(), "FindEntityByName must be called from main thread");
	for (u_int u = 0; u < m_xActiveEntities.GetSize(); ++u)
	{
		Zenith_EntityID xID = m_xActiveEntities.Get(u);
		if (EntityExists(xID))
...
{
	Zenith_Assert(Zenith_ECS_IsMainThread(), "SetMainCameraEntity must be called from main thread");
...
// GetMainCamera() / TryGetMainCamera() (the Zenith_CameraComponent-returning
// accessors) moved ENGINE-side -- they forced the concrete component include
// into this ECS-core TU. The replacements live in the leaf-clean resolver
// EntityComponent/Zenith_CameraResolve.{h,cpp}: Zenith_GetMainCamera(this) and
// Zenith_TryGetMainCamera(this), with identical semantics (same EntityID, same
// GetComponentFromEntity<Zenith_CameraComponent>, same null/assert behaviour).
...
{
	Zenith_Assert(Zenith_ECS_IsMainThread(), "MarkForDestruction must be called from main thread");
	if (!xID.IsValid()) return;
	if (!EntityExists(xID)) return;

	// Already marked - prevent double-marking and infinite recursion (use global slot flag)
	if (Zenith_ECS_EntityStore().m_axEntitySlots.Get(xID.m_uIndex).m_bMarkedForDestruction) return;
	...
	if (Zenith_ECS_EntityStore().m_axEntitySlots.Get(xID.m_uIndex).IsPendingStart())
	{
	...
	Zenith_ECS_EntityStore().m_axEntitySlots.Get(xID.m_uIndex).TransitionToDestroying();
	m_xPendingDestruction.PushBack(xID);
}

void Zenith_SceneData::MarkChildrenForDestructionRecursive(Zenith_EntityID xID)
{
	// Phase 5a: children come from the SLOT (single source of truth). Copy the list
	// before recursing — the recursion mutates slot flags but not this vector, yet
	// copying keeps iteration robust against any future in-loop mutation (matches
	// the prior copy-then-iterate of the owning component's child list).
	if (xID.m_uIndex >= Zenith_ECS_EntityStore().m_axEntitySlots.GetSize()) return;
	Zenith_Vector<Zenith_EntityID> axChildIDs = Zenith_ECS_EntityStore().m_axEntitySlots.Get(xID.m_uIndex).m_xChildEntityIDs;
	for (u_int u = 0; u < axChildIDs.GetSize(); ++u)
	{
		Zenith_EntityID xChildID = axChildIDs.Get(u);
		if (xChildID.IsValid() && EntityExists(xChildID) && !Zenith_ECS_EntityStore().m_axEntitySlots.Get(xChildID.m_uIndex).m_bMarkedForDestruction)
		{
			Zenith_ECS_EntityStore().m_axEntitySlots.Get(xChildID.m_uIndex).TransitionToDestroying();
...
{
	Zenith_Assert(Zenith_ECS_IsMainThread(), "MarkForTimedDestruction must be called from main thread");
...
bool Zenith_SceneData::IsMarkedForDestruction(Zenith_EntityID xID) const
{
	...
	return Zenith_ECS_EntityStore().m_axEntitySlots.Get(xID.m_uIndex).m_bMarkedForDestruction;
}

void Zenith_SceneData::ProcessPendingDestructions()
{
	Zenith_Assert(Zenith_ECS_IsMainThread(), "ProcessPendingDestructions must be called from main thread");
	for (int i = static_cast<int>(m_xPendingDestruction.GetSize()) - 1; i >= 0; --i)
	{
		...
		if (EntityExists(xEntityID))
		{
			...
			// Phase 5b: detach via the slot-backed Zenith_Entity API (the hierarchy
			// is owned by the slot). Behaviour-identical to the former component-side
...
void Zenith_SceneData::DispatchAwakeAndEnableForNewEntities(Zenith_Vector<Zenith_EntityID>& axAllNewEntities)
{
	...
	while (m_axNewlyCreatedEntities.GetSize() > 0 && uIteration < uMAX_AWAKE_ITERATIONS)
	{
		...
		for (u_int u = 0; u < axNewEntities.GetSize(); ++u)
		{
			...
			Zenith_EntitySlot& xSlot = Zenith_ECS_EntityStore().m_axEntitySlots.Get(uID.m_uIndex);
...
void Zenith_SceneData::ClearCreatedDuringUpdateFlags()
{
	for (u_int u = 0; u < m_xActiveEntities.GetSize(); ++u)
	{
		...
		if (xID.m_uIndex < Zenith_ECS_EntityStore().m_axEntitySlots.GetSize())
		{
			Zenith_ECS_EntityStore().m_axEntitySlots.Get(xID.m_uIndex).m_bCreatedDuringUpdate = false;
		}
	}
}
...
{
	Zenith_Assert(Zenith_ECS_IsMainThread(), "FixedUpdate must be called from main thread");
	Zenith_ComponentMetaRegistry& xRegistry = Zenith_ComponentMetaRegistry::Get();
...
void Zenith_SceneData::DispatchImmediateLifecycleForRuntime(Zenith_EntityID xID)
{
	...
	if (Zenith_SceneSystem::Get().IsLoadingScene() || Zenith_SceneSystem::Get().IsPrefabInstantiating())
		return;
...
void Zenith_SceneData::DispatchAwakeForNewScene()
{
	...
	if (!m_strPath.empty())
	{
		Zenith_SceneSystem::Get().PushLifecycleContext(m_strPath);
	}

	// Phase 1: OnAwake for all entities (Unity ordering: Awake and OnEnable run before Start)
	...
	if (!m_strPath.empty())
	{
		Zenith_SceneSystem::Get().PopLifecycleContext(m_strPath);
	}
}
...
{
	if (xEntityID.m_uIndex >= Zenith_ECS_EntityStore().m_axEntitySlots.GetSize()) return PendingStartResult::SKIP;
	Zenith_EntitySlot& xSlot = Zenith_ECS_EntityStore().m_axEntitySlots.Get(xEntityID.m_uIndex);
	...
	{
		Zenith_SceneData* pxTargetData = Zenith_SceneSystem::Get().GetSceneDataByHandle(xSlot.m_iSceneHandle);
		if (pxTargetData)
		{
