...
### The deserialize thunk, and the component REFUSAL channel
`ComponentDeserializeFn` is `bool(*)(Zenith_Entity&, Zenith_DataStream&, u_int)` and `DeserializeEntityComponents` returns `bool`. The bool is the COMPONENT's verdict on the bytes it was handed — the only way a component can say "this schema is not mine". A component opts in by giving its **versioned** reader a `bool` return:

```cpp
static constexpr u_int uSchemaVersion = 3;
bool ReadFromDataStream(Zenith_DataStream& xStream, u_int uPersistedSchemaVersion);  // false = refuse
```

`ComponentDeserializeWrapper<T>` picks the branch with concepts: `HasBoolVersionedReadFromDataStream<T>` (returns the reader's verdict) → `HasVersionedReadFromDataStream<T>` (void 2-arg reader; `true`) → the 1-arg reader (`true`). **The two versioned concepts are disjoint on the RETURN TYPE** — `HasVersionedReadFromDataStream` is constrained `-> std::same_as<void>` for exactly that reason: it used to be return-unconstrained, so a bool reader satisfied it too and whichever branch came first in the `if constexpr` chain would have discarded the refusal silently. A `void` reader can only succeed, which is why none of the ~90 shipped components needed changing.

**RECORD AND CONTINUE — a refusal is never an early return.** `DeserializeEntityComponents` `&=`s (non-short-circuiting) each component's verdict, logs one `Zenith_Error` naming the type and the entity, and **still runs the bounded realign** `SetCursor(payloadStart + declaredSize)` for the refusing component and **still visits every remaining component**. So a refusing reader may bail on the version word without consuming a byte and the next component — and the next entity — still read intact. The verdict travels `component → DeserializeEntityComponents → Zenith_SceneData::ReadEntityFromDataStream → LoadFromDataStream` (which finishes every entity, the hierarchy pass, the reference-resolve pass and the camera trailer before returning it), then `LoadFromFile` → `Zenith_SceneSystem::LoadScene`, which is the layer that rolls back with `UnloadSceneForced`.

**An UNKNOWN component type is NOT a refusal.** It stays warn-and-skip (the size prefix exists for it), because a file written by a build with more components than this one is a supported read. Failing the load there would break every cut-down build. The refusal channel is for a KNOWN component that cannot read a KNOWN-to-be-different schema.

Other consumers of the verdict: `Zenith_Entity::ReadFromDataStream`, `Zenith_Prefab::DeserializeComponents`/`ApplyToEntity` (`Instantiate` logs and **still returns the entity** — a half-populated entity beats a leaked spawn), and the editor's undo byte-restore (`Zenith_EditorCommands.cpp`), which logs and continues so a subtree restore is never left half-applied. The thunk is deliberately **not** `[[nodiscard]]` for that last call site.
...
## Key Concepts
**Serialization:** components implement `WriteToDataStream()`/`ReadFromDataStream()`; the ComponentMeta system enforces serialization order (lower = first). A component may additionally implement `bool ReadFromDataStream(stream, uSchemaVersion)` to REFUSE a schema it cannot read — see [the refusal channel](#the-deserialize-thunk-and-the-component-refusal-channel); the refusal is recorded and the load continues, and `LoadFromDataStream` reports false once at the end. **Entities are referenced in a scene file by a DENSE, authoring-order file index** (`Zenith_EntityFileIndexMap`), never by their `EntityID` slot index: slots come from a process-global table, so writing one would make a scene's bytes depend on how many entities that particular boot allocated first — which is exactly what used to stop `.zscen` files being committed to git. The loader treats file indices as opaque keys, so older files (whose indices are sparse slot indices) still load. Pinned by `Scene::SceneBytesAreIndependentOfSlotAllocation`. Scene/prefab format is v7 (v6→v7 migration reads the legacy parent file-index out of the owning component's payload and routes it through `Zenith_Entity::SetPendingParentFileIndex` during component deserialization, then resolves it inline in the hierarchy-rebuild loop in `Internal/Zenith_SceneData_Serialization.cpp` — `GetPendingParentFileIndex` → `SetParent` → `ClearPendingParentFileIndex`; there is no method named `ResolvePendingParents`, that is just the informal name the comments use for this pass).
