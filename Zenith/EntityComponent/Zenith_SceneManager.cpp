...
#if 0 // bodies migrated to Zenith_SceneOperationQueue.cpp
Zenith_SceneOperationID Zenith_SceneManager::LoadSceneAsync_OLD(const std::string& strPath, Zenith_SceneLoadMode eMode)
{
	...
	if (eMode == SCENE_LOAD_ADDITIVE_WITHOUT_LOADING)
	{
		...
		uint64_t ulOpID = g_xEngine.SceneOperations().AllocateOperationID();
	...
	if (!Zenith_FileAccess::FileExists(strPath.c_str()))
	{
		...
		g_xEngine.SceneOperations().FailAsyncLoadOperation(pxOp);
		return ulOpID;
	}
...
#endif // 0 — LoadSceneAsync/LoadSceneAsyncByIndex bodies migrated to Zenith_SceneOperationQueue.cpp
