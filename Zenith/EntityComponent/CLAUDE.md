...
**Purpose:** Renderable mesh with material and physics mesh support

```cpp
class Zenith_ModelComponent {
public:
    // Mesh entry system - supports multiple meshes per component
    struct MeshEntry {
        Flux_MeshGeometry* m_pxGeometry;
        Flux_MaterialAsset* m_pxMaterial;
    };

    void AddMeshEntry(Flux_MeshGeometry& xGeometry, Flux_MaterialAsset& xMaterial);
    uint32_t GetNumMeshEntries() const;

    // Physics mesh for collision and selection
    Flux_MeshGeometry* GetPhysicsMesh() const { return m_pxPhysicsMesh; }
    void GeneratePhysicsMesh(PhysicsMeshQuality eQuality = PHYSICS_MESH_QUALITY_HIGH);
...
private:
    ...
    Flux_MeshGeometry* m_pxPhysicsMesh;  // Simplified mesh for physics/selection
    Zenith_Entity m_xParentEntity;

    // Asset lifetime tracking (December 2024)
...
#### Physics Mesh System (December 2024)

**Purpose:** Physics meshes provide simplified collision geometry and accurate entity selection in the editor.

**Auto-Generation:** Controlled by global config `g_xPhysicsMeshConfig` in `Zenith_PhysicsMeshGenerator.cpp`:

```cpp
PhysicsMeshConfig g_xPhysicsMeshConfig = {
    PHYSICS_MESH_QUALITY_HIGH,  // Use full mesh geometry (no simplification)
    1.0f,                        // Simplification ratio
    100,                         // Min triangles
    10000,                       // Max triangles
    true,                        // m_bAutoGenerate: Auto-generate on deserialization
    true,                        // m_bDebugDraw: Render wireframe visualization
    Zenith_Maths::Vector3(0.0f, 1.0f, 0.0f)  // Debug color (green)
};
```

**CRITICAL: Position Data Retention**

Physics mesh generation requires vertex position data to be retained in CPU memory. When deserializing model components from scene files, the `uRetainFlags` parameter MUST include `FLUX_VERTEX_ATTRIBUTE__POSITION`:
...
Flux_MeshGeometry* pxMesh = Zenith_AssetHandler::AddMeshFromFile(
    strMeshPath.c_str(),
    uRetainFlags,  // Without this, physics mesh generation FAILS
    true
);
```

Without this flag, position data is deleted after GPU upload, making physics mesh generation impossible.

**Deserialization Flow:**

1. `ReadFromDataStream()` loads mesh paths and materials
2. Meshes are loaded with position data retained (`uRetainFlags`)
3. If `g_xPhysicsMeshConfig.m_bAutoGenerate` is true, `GeneratePhysicsMesh()` is called
4. Physics mesh is generated by combining/simplifying all mesh entries
5. Debug logging confirms success/failure

**Editor Integration:**

- **Selection:** `Zenith_SelectionSystem` uses physics mesh for triangle-level raycasting (pixel-perfect selection)
- **Visualization:** Green wireframe overlay via `Flux_Primitives` (visible in all editor modes)
- **Debug Draw:** Enabled by default via `g_xPhysicsMeshConfig.m_bDebugDraw`

See [Editor/CLAUDE.md - Physics Mesh Debug Visualization](../Editor/CLAUDE.md#physics-mesh-debug-visualization) for full details.
