...
// bootstrap.
//
// Instance methods reach their own members directly (this == &g_xEngine.Scenes()).
// The static bootstrap methods and the non-member RAII guards have no `this`,
// so they reach the singleton through g_xEngine.Scenes().
//=============================================================================
...
void Zenith_SceneSystem::Shutdown()
{
	...
	// Callback bus state.
	m_xActiveSceneChangedCallbacks.m_axEntries.Clear();
	m_xSceneLoadedCallbacks.m_axEntries.Clear();
	m_xSceneUnloadingCallbacks.m_axEntries.Clear();
	m_xSceneUnloadedCallbacks.m_axEntries.Clear();
	m_axCallbacksPendingRemoval.Clear();
	m_uFiringCallbacksDepth = 0;
	m_bSuppressActiveSceneChanged = false;
