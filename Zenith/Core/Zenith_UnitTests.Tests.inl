...
#include "Core/Zenith_CommandLine.h"   // TestProceduralTreeAssetExport: --skip-tool-exports gate
...
void Zenith_UnitTests::TestStickFigureMeshCreation(){
	...
	// Skip when tool exports were bypassed (--skip-tool-exports): the StickFigure
	// asset set is written by GenerateTestAssets and is absent on a from-scratch
	// headless CI boot. See TestProceduralTreeAssetExport for the full rationale.
	if (Zenith_CommandLine::IsToolExportSkipped())
	{
		ZENITH_SKIP("--skip-tool-exports: StickFigure assets were not generated this run");
	}
	...
	// Weights normalized; bone indices reference the exported rig — now the 16
	...
	for (uint32_t v = 0; v < pxMesh->GetNumVerts(); v++)
	{
		...
		ZENITH_ASSERT_TRUE(xIndices.x < uRigBones && xIndices.y < uRigBones,
			"Bone indices must reference the rig");
	}
...
void Zenith_UnitTests::TestStickFigureAssetExport(){
	...
	// The exported rig keeps the 16 core bones and APPENDS UE5-class bones
	// (jaw/eyes/toes + articulated fingers). The mesh + Flux geometry must agree
	// with the skeleton's actual bone count (captured after reload below).
 ...
 * Assets are generated by GenerateTestAssets() called earlier in main().
 * The branching tree (Zenith_Tools_TreeAssetExport.cpp) is seeded, so its
 * structure is deterministic — but the test pins INVARIANTS and
 * cross-consistency (mesh == geometry == VAT vertex counts) rather than
 * exact magic counts, so generator tuning doesn't churn this test.
 */
#ifndef ZENITH_ANDROID // Asset verification uses std::filesystem with local paths
...
void Zenith_UnitTests::TestProceduralTreeAssetExport(){
	...
	// The procedural tree asset set is written by GenerateTestAssets() during
	// Zenith_Engine::InitialiseAssets, which is bypassed under --skip-tool-exports
	// (the from-scratch CI unit-gate boots Sokoban that way, and Zenith/Assets/ is
	// gitignored -- nothing is on disk to reload). Verifying assets that were
	// deliberately never generated would assert-fail (and can wedge the loader on
	// the missing dependency), so report a clean skip. When exports DO run, a
	// genuinely-missing asset stays a real regression and still fails below.
	if (Zenith_CommandLine::IsToolExportSkipped())
	{
		ZENITH_SKIP("--skip-tool-exports: procedural tree assets were not generated this run");
	}
	...
	// Full generated file set: skeleton, split trunk/leaves meshes + VATs,
	// textures (incl. the alpha-mapped leaf cluster), and the materials.
	const char* aszExpectedFiles[] = {
		"Tree" ZENITH_SKELETON_EXT,
		"Tree" ZENITH_MESH_ASSET_EXT,
		"Tree_Trunk" ZENITH_MESH_ASSET_EXT,
		"Tree_Leaves" ZENITH_MESH_ASSET_EXT,
		"Tree_Trunk_Sway.zanmt",
		"Tree_Leaves_Sway.zanmt",
		"Tree_Bark_Albedo" ZENITH_TEXTURE_EXT,
		"Tree_Bark_Normal" ZENITH_TEXTURE_EXT,
		"Tree_Leaves_Albedo" ZENITH_TEXTURE_EXT,
		"Tree_Bark.zmtrl",
		"Tree_Leaves.zmtrl",
		"Tree_Sway" ZENITH_ANIMATION_EXT,
	};
	for (const char* szFile : aszExpectedFiles)
	{
		ZENITH_ASSERT_TRUE(std::filesystem::exists(strOutputDir + szFile), "Generated tree asset missing: %s", szFile);
	}
	...
	ZENITH_ASSERT_TRUE(pxReloadedSkel->HasBone("TreeRoot"), "Reloaded skeleton should have TreeRoot bone");
...
#ifdef ZENITH_TOOLS
	// VAT <-> mesh vertex-count consistency (the instanced shader indexes the
	// VAT by SV_VertexID, so any mismatch silently corrupts the sway).
	Flux_AnimationTexture* pxTrunkVAT = Flux_AnimationTexture::LoadFromFile(strOutputDir + "Tree_Trunk_Sway.zanmt");
	ZENITH_ASSERT_NOT_NULL(pxTrunkVAT, "Should be able to reload trunk VAT");
	ZENITH_ASSERT_EQ(pxTrunkVAT->GetVertexCount(), pxTrunkMesh->GetNumVerts(), "Trunk VAT vertex count mismatch");
	ZENITH_ASSERT_EQ(pxTrunkVAT->GetNumAnimations(), 1, "Trunk VAT should have 1 animation");
	delete pxTrunkVAT;

	Flux_AnimationTexture* pxLeavesVAT = Flux_AnimationTexture::LoadFromFile(strOutputDir + "Tree_Leaves_Sway.zanmt");
	ZENITH_ASSERT_NOT_NULL(pxLeavesVAT, "Should be able to reload leaves VAT");
	ZENITH_ASSERT_EQ(pxLeavesVAT->GetVertexCount(), pxLeavesMesh->GetNumVerts(), "Leaves VAT vertex count mismatch");
	delete pxLeavesVAT;
#endif
...
void Zenith_UnitTests::TestRenderGraphAliasingTopoOrderInterleaved(){
	...
	ZENITH_ASSERT_EQ(pxT_A->m_uFirstWriteTopoIdx, 0u, "T_A first-write at topo 0");
	ZENITH_ASSERT_EQ(pxT_A->m_uLastUseTopoIdx,    2u, "T_A last-use at topo 2 (PA2 read)");
	ZENITH_ASSERT_EQ(pxT_B->m_uFirstWriteTopoIdx, 1u, "T_B first-write at topo 1");
	ZENITH_ASSERT_EQ(pxT_B->m_uLastUseTopoIdx,    3u, "T_B last-use at topo 3 (PB2 read)");
...
// Pre-fix, m_uLastUseTopoIdx = m_uLastReadTopoIdx = 1 (P1's read), missing the later
// write at P2. Post-fix, m_uLastUseTopoIdx = max(last_read, last_write) = 2.
...
void Zenith_UnitTests::TestRenderGraphMultiWriteLastUse(){
	...
	ZENITH_ASSERT_EQ(pxT->m_uFirstWriteTopoIdx, 0u, "first-write at topo 0");
	ZENITH_ASSERT_EQ(pxT->m_uLastUseTopoIdx, 2u,
		"REGRESSION GUARD: m_uLastUseTopoIdx must cover the LATER write at topo 2, not stop at the read at topo 1. "
		"Pre-fix, only m_uFirstWriteTopoIdx + m_uLastReadTopoIdx were tracked; the second write was invisible to the packer, "
...
void Zenith_UnitTests::TestRenderGraphAliasingMultiWriteOverlap(){
	...
	ZENITH_ASSERT_EQ(pxT1->m_uLastUseTopoIdx, 4u,
		"T1 last-use must extend through the second write at topo 4 (last_write=4 > last_read=1)");
	ZENITH_ASSERT_EQ(pxT2->m_uFirstWriteTopoIdx, 2u, "T2 first-write at topo 2");
	ZENITH_ASSERT_EQ(pxT2->m_uLastUseTopoIdx,    3u, "T2 last-use at topo 3");
	...
	ZENITH_ASSERT_NE(pxT1->m_uAliasPoolIndex, pxT2->m_uAliasPoolIndex,
		...
		"If this fires, m_uLastUseTopoIdx is back to ignoring multi-write extensions and T2's writes will trample "
...
void Zenith_UnitTests::TestRenderGraphDisabledPassExcludedFromLifetimes(){
	...
	Flux_RenderGraph::TransientResource* pxT = xGraph.m_axTransients.Get(xT.m_uIndex);
	ZENITH_ASSERT_EQ(pxT->m_uLastUseTopoIdx, 2u, "all enabled: last-use at topo 2");
	...
	ZENITH_ASSERT_EQ(pxT->m_uLastUseTopoIdx, 1u,
...
// drift in m_uLastWriteTopoIdx tracking when entries already exist in m_xResources).
...
void Zenith_UnitTests::TestRenderGraphLifetimeRecomputeIdempotent(){
	...
	Flux_RenderGraph::TransientResource* pxT = xGraph.m_axTransients.Get(xT.m_uIndex);
	const u_int uFirstWrite_1 = pxT->m_uFirstWriteTopoIdx;
	const u_int uLastUse_1    = pxT->m_uLastUseTopoIdx;
	...
	ZENITH_ASSERT_EQ(pxT->m_uFirstWriteTopoIdx, uFirstWrite_1,
		"ComputeResourceLifetimes must be idempotent — first-write drifted between calls (was %u, now %u)",
		uFirstWrite_1, pxT->m_uFirstWriteTopoIdx);
	ZENITH_ASSERT_EQ(pxT->m_uLastUseTopoIdx, uLastUse_1,
		"ComputeResourceLifetimes must be idempotent — last-use drifted between calls (was %u, now %u)",
		uLastUse_1, pxT->m_uLastUseTopoIdx);
	ZENITH_ASSERT_EQ(pxT->m_uFirstWriteTopoIdx, 0u, "first-write at topo 0");
	ZENITH_ASSERT_EQ(pxT->m_uLastUseTopoIdx,    2u, "last-use at topo 2 (multi-write)");
}

// SynthesizeAliasingBarriers must translate m_uLastUseTopoIdx (topological order) back
...
// keyed by pass index. A regression that used m_uLastUseTopoIdx directly as a pass
...
void Zenith_UnitTests::TestRenderGraphAliasingBarrierUsesTopologicalLastUse(){
	...
	ZENITH_ASSERT_EQ(xBar.m_eSrcAccess, RESOURCE_ACCESS_READ_SRV,
		...
		"If this returns WRITE_RTV, SynthesizeAliasingBarriers used pxPrior->m_uLastUseTopoIdx directly as a pass index "
...
ZENITH_TEST(Flux, PersistentSetLayouts)
{
	...
	// + binding 2 g_xShadowMatrices storage buffer.
	...
	{
		Flux_BindingGroupLayout x = fnViewCanonical();
		x.m_axBindings[2].m_bPresent = false; // drop g_xShadowMatrices (Phase 5.4)
		ZENITH_ASSERT_TRUE(!ValidateCanonicalGroup(FLUX_FREQUENCY_CLASS_VIEW, x, strErr), "VIEW missing the g_xShadowMatrices SSBO rejected");
	}
	{
		Flux_BindingGroupLayout x = fnViewCanonical();
		x.m_axBindings[2].m_eKind = FLUX_RESOURCE_KIND_COMBINED_TEXTURE_SAMPLER; // wrong kind at binding 2 (must be a storage buffer)
		ZENITH_ASSERT_TRUE(!ValidateCanonicalGroup(FLUX_FREQUENCY_CLASS_VIEW, x, strErr), "VIEW binding 2 wrong kind rejected");
	}
	{
		Flux_BindingGroupLayout x = fnViewCanonical();
...
// ============================================================================
// FrameContext frame-index tests
// ============================================================================
// These tests run inside the live engine's main loop (RunAllTests is invoked
// from Zenith_Main.cpp). FrameContext (g_xEngine.Frame()) owns the single
// frame-index variable engine-wide; the tests drive it directly via
// AdvanceFrameIndex and save/restore the live index at entry/exit so the
// surrounding frame loop is unaffected. They deliberately do NOT call
// BeginFrame/ProcessFrameEnd, which would issue real backend per-frame work
// (fence waits) outside a real frame.

#include "Flux/Flux_PerFrame.h"
#include "Flux/Flux_RendererImpl.h"

// Snapshot of FrameContext's frame index used by the frame-index tests to
// save the live engine state before driving the index, then restore it at
// end-of-test.
...
{
	xOut.m_uFrameIndex = g_xEngine.Frame().m_uFrameIndex;
	g_xEngine.Frame().m_uFrameIndex = 0;
}

void Zenith_UnitTests::RestorePerFrame(const PerFrameSnapshot& xIn)
{
	g_xEngine.Frame().m_uFrameIndex = xIn.m_uFrameIndex;
}
...
void Zenith_UnitTests::TestFluxPerFrameFrameCounterAdvances(){
	...
	ZENITH_ASSERT_EQ(g_xEngine.Frame().GetFrameIndex(), 0, "Frame index starts at 0");
	ZENITH_ASSERT_EQ(g_xEngine.Frame().GetRingIndex(), 0, "Ring index starts at 0");

	// AdvanceFrameIndex bumps the monotonic frame index by one (Zenith_MainLoop
	// calls it once per frame, after the end-of-frame work). Driven directly
	// here so the test doesn't issue real backend per-frame work (fence waits).
	g_xEngine.Frame().AdvanceFrameIndex();
	ZENITH_ASSERT_EQ(g_xEngine.Frame().GetFrameIndex(), 1, "AdvanceFrameIndex advances the frame index by 1");

	for (u_int u = 0; u < 5; u++)
	{
		g_xEngine.Frame().AdvanceFrameIndex();
	}
	ZENITH_ASSERT_EQ(g_xEngine.Frame().GetFrameIndex(), 6, "Five more advances take the frame index to 6");
...
void Zenith_UnitTests::TestFluxPerFrameRingIndexWraps(){
	...
	{
		ZENITH_ASSERT_EQ(g_xEngine.Frame().GetRingIndex(), u % MAX_FRAMES_IN_FLIGHT, "Ring index at frame %u is %u, expected %u", u, g_xEngine.Frame().GetRingIndex(), u % MAX_FRAMES_IN_FLIGHT);
		g_xEngine.Frame().AdvanceFrameIndex();
	}
	// After MAX_FRAMES_IN_FLIGHT advances the index is at MAX, ring index wraps to 0.
	ZENITH_ASSERT_EQ(g_xEngine.Frame().GetFrameIndex(), MAX_FRAMES_IN_FLIGHT, "Frame index is at MAX_FRAMES_IN_FLIGHT");
	ZENITH_ASSERT_EQ(g_xEngine.Frame().GetRingIndex(), 0, "Ring index wraps to 0 after MAX_FRAMES_IN_FLIGHT advances");
	...
	ZENITH_ASSERT_EQ(g_xEngine.Frame().GetFrameIndex(), uExpectedCounter, "Frame index total tracks correctly");
	ZENITH_ASSERT_EQ(g_xEngine.Frame().GetRingIndex(), uExpectedCounter % MAX_FRAMES_IN_FLIGHT, "Ring index continues to be frame index %% MAX_FRAMES_IN_FLIGHT");
